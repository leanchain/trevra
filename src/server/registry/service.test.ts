...
import { listCommercialProjections, rebuildCommercialProjections, runCommercialProjectionCycle } from '../projections/commercial.js';
import {
...
async function verifiedPublisher(database: Db, workspaceId: string, userId: string) {
  ...
  const publisher = await createModulePublisher(database, {
    workspaceId, userId, slug, displayName: 'Test Publisher',
...
function manifestFor(slug: string, overrides: Record<string, unknown> = {}) {
  return {
    id: `${slug}.remote`, version: '1.0.0', name: 'Remote test', description: 'Exercise signed sandbox execution.',
    ...
    permissions: { network: ['module.example'], secrets: [] as string[], filesystem: 'none' as const },
...
function signManifest(manifest: Record<string, unknown>, privateKey: ReturnType<typeof generateKeyPairSync>['privateKey']): string {
  return sign(null, Buffer.from(moduleReleaseSigningPayload(manifest as never, testSbom)), privateKey).toString('base64');
}
...
describe('hosted module registry',()=>{
  ...
  it('publishes as draft until an operator has reviewed the publisher',async()=>{
    ...
    const slug=`pub-${id('t').slice(2,14)}`;
    const publisher=await createModulePublisher(database,{
      workspaceId:DEMO_WORKSPACE_ID,userId:DEMO_USER_ID,slug,displayName:'Unreviewed Publisher',
      publicKeyPem:publicKey.export({type:'spki',format:'pem'}).toString()
    });
    const manifest=manifestFor(slug);
    const release=await publishModuleRelease(database,{
      workspaceId:DEMO_WORKSPACE_ID,userId:DEMO_USER_ID,publisherId:publisher.id,
      manifest,signature:signManifest(manifest,privateKey),sbom:testSbom
    });

    expect(release.status).toBe('draft');
    await expect(installModuleRelease(database,{
      workspaceId:DEMO_WORKSPACE_ID,userId:DEMO_USER_ID,moduleId:manifest.id,version:manifest.version
  ...
  it('refuses to publish a module id outside the publisher namespace',async()=>{
    ...
    await expect(publishModuleRelease(database,{
      workspaceId:DEMO_WORKSPACE_ID,userId:DEMO_USER_ID,publisherId:publisher.id,
      manifest:squatted,signature:signManifest(squatted,publisher.privateKey),sbom:testSbom
  ...
  it('refuses to install another tenant private module, and says only that it is not there',async()=>{
    ...
    try{
      ...
      await publishModuleRelease(database,{
        workspaceId:DEMO_WORKSPACE_ID,userId:DEMO_USER_ID,publisherId:publisher.id,
        manifest,signature:signManifest(manifest,publisher.privateKey),sbom:testSbom
      });
      await database.prepare("UPDATE module_packages SET visibility='private' WHERE module_id=?").run(manifest.id);

      await expect(installModuleRelease(database,{
        workspaceId:intruderWorkspace,userId:DEMO_USER_ID,moduleId:manifest.id,version:manifest.version
      })).rejects.toThrow('Verified module release not found');
      const installed=await database.prepare('SELECT COUNT(*)::int AS total FROM workspace_module_installations WHERE workspace_id=?')
      ...
      await installModuleRelease(database,{workspaceId:DEMO_WORKSPACE_ID,userId:DEMO_USER_ID,moduleId:manifest.id,version:manifest.version});
      expect(await getInstalledCommunityModule(database,DEMO_WORKSPACE_ID,manifest.id)).not.toBeNull();
    }finally{
...
describe('commercial projections',()=>{
  ...
  it('rebuilds one tenant without touching another tenant projections',async()=>{
    ...
    await database.prepare('INSERT INTO workspaces (id,name,created_at) VALUES (?,?,?)').run(otherWorkspace,'Neighbour',now);
    try{
      await database.prepare('INSERT INTO clients (id,workspace_id,name,contact_name,email,status,active_value,currency,last_interaction_at,created_at) VALUES (?,?,?,?,?,?,?,?,?,?)')
        .run(otherClient,otherWorkspace,'Neighbour Co','Pat','pat@neighbour.test','active',0,'EUR',now,now);
      await runCommercialProjectionCycle(database,5000);
      expect((await listCommercialProjections(database,otherWorkspace,{entityType:'clients'})).length).toBe(1);
      const demoBefore=await listCommercialProjections(database,DEMO_WORKSPACE_ID,{entityType:'clients'});
      ...
      const neighbour=await listCommercialProjections(database,otherWorkspace,{entityType:'clients'});
      expect(neighbour.length).toBe(1);
      expect(neighbour[0]!.state.name).toBe('Neighbour Co');
      const demoAfter=await listCommercialProjections(database,DEMO_WORKSPACE_ID,{entityType:'clients'});
      expect(demoAfter.map((row)=>row.entityId).sort()).toEqual(demoBefore.map((row)=>row.entityId).sort());
      ...
      const checkpoint=await database.prepare('SELECT last_position FROM projection_checkpoints WHERE projection_name=?')
