...
function stateWith(
  overrides: Partial<BrowserStorageState> = {},
  ...
  return {
    ...
    origins: [
      { origin: 'https://www.linkedin.com', localStorage: [{ name: 'voyager', value: 'x' }] }
    ],
    ...overrides
  };
}
...
beforeAll(() => {
  ...
  previousProvider = process.env.TREVRA_BROWSER_PROVIDER;
  previousEndpoint = process.env.TREVRA_BROWSER_CDP_URL;
  previousDeploymentMode = process.env.TREVRA_DEPLOYMENT_MODE;

  process.env.TREVRA_SECRETS_KEY = randomBytes(32).toString('base64');
  process.env.TREVRA_BROWSER_PROVIDER = 'remote';
  process.env.TREVRA_BROWSER_CDP_URL = 'wss://connect.example.com/';
  process.env.TREVRA_DEPLOYMENT_MODE = 'local';
});

afterAll(() => {
  if (previousKey === undefined) delete process.env.TREVRA_SECRETS_KEY;
  else process.env.TREVRA_SECRETS_KEY = previousKey;
  if (previousProvider === undefined) delete process.env.TREVRA_BROWSER_PROVIDER;
  else process.env.TREVRA_BROWSER_PROVIDER = previousProvider;
  if (previousEndpoint === undefined) delete process.env.TREVRA_BROWSER_CDP_URL;
  else process.env.TREVRA_BROWSER_CDP_URL = previousEndpoint;
...
describe('a stored browser session', () => {
  it('round-trips exactly, and nothing about it is in the row in the clear', async () => {
    ...
    const row = await db
      .prepare('SELECT * FROM linkedin_seat_sessions WHERE workspace_id=? AND seat_key=?')
  ...
  it("is bound to its seat: another seat's row cannot be opened as this one", async () => {
    ...
    const source = await db
      .prepare(
        'SELECT ciphertext, iv, auth_tag, key_version, key_id FROM linkedin_seat_sessions WHERE workspace_id=? AND seat_key=?'
      )
      .get<Record<string, unknown>>(WORKSPACE_ID, 'sales');
...
describe('a session that cannot be used', () => {
  it('degrades to needs_login when it has expired, and says when', async () => {
    await saveSeatStorageState(db, {
      workspaceId: WORKSPACE_ID,
      seatKey: 'sales',
      state: stateWith({}, 1_700_000_000),
      now: NOW
    });
    const read = await readSeatStorageState(db, WORKSPACE_ID, 'sales', {
      now: new Date(1_700_000_001 * 1000)
    });
    expect(read.status).toBe('needs_login');
    if (read.status !== 'needs_login') throw new Error('unreachable');
  ...
  it('refuses to store a state that is not signed in, rather than overwriting a good one', async () => {
    ...
    const unauthenticated: BrowserStorageState = {
      cookies: [{ name: 'bcookie', value: 'v' }],
      origins: []
    };
  ...
  it('degrades to needs_login when the sealed bytes are not a storage state at all', async () => {
    const sealed = sealSecret(
      ...
      sessionStateContext(WORKSPACE_ID, 'sales'),
      process.env
    );
    await db
      .prepare(
  ...
  it('reports absent -- not needs_login -- when there simply is no row', async () => {
    expect((await readSeatStorageState(db, WORKSPACE_ID, 'never-seen', { now: NOW })).status).toBe(
      'absent'
    );
  });
  ...
  it('is forgotten on demand, so a challenged session is not replayed', async () => {
    ...
    expect((await readSeatStorageState(db, WORKSPACE_ID, 'sales', { now: NOW })).status).toBe(
      'absent'
    );
