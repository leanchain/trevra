...
beforeEach(async () => {
  ...
  await db
    ...
    .run(WORKSPACE, 'Manager Test', NOW.toISOString());
  ...
  const result = await importLeadCsv(
    db,
    {
      ...
      csv: [
        ...
        'Dr. Maya,Smith,Acme,maya@acme.test,https://linkedin.com/in/maya-smith',
        'Jon,Jones,Widgets,jon@widgets.test,https://linkedin.com/in/jon-jones'
      ].join('\n')
    },
    NOW
  );
...
describe('LinkedIn outreach manager persistence', () => {
  it('imports canonical contacts and applies the same scrubber on edits', async () => {
    ...
    const updated = await updateLeadContact(
      db,
      {
        workspaceId: WORKSPACE,
        contactId: maya!.id,
  ...
  it('enforces one active campaign per contact even when the member is paused', async () => {
    ...
    const first = await createManagedCampaign(
      db,
      { workspaceId: WORKSPACE, name: 'One', leadListId: listId, workflowId },
      NOW
    );
    ...
    const second = await createManagedCampaign(
      db,
      { workspaceId: WORKSPACE, name: 'Two', leadListId: listId, workflowId },
      NOW
    );
    ...
    const third = await createManagedCampaign(
      db,
      { workspaceId: WORKSPACE, name: 'Three', leadListId: listId, workflowId },
      NOW
    );
  ...
  it('starts, pauses and stops a managed campaign without sending anything', async () => {
    ...
    const made = await createManagedCampaign(
      db,
      { workspaceId: WORKSPACE, name: 'Lifecycle', leadListId: listId, workflowId },
      NOW
    );
    const running = await startManagedCampaign(db, WORKSPACE, made.campaign.id, NOW);
    expect(running.status).toBe('running');
    expect(
      (await listCampaignMembers(db, WORKSPACE, made.campaign.id)).every(
        (member) => member.status === 'pending' && member.admittedAt === null
      )
    ).toBe(true);
    expect((await pauseManagedCampaign(db, WORKSPACE, made.campaign.id, NOW)).status).toBe(
      'paused'
    );
    expect((await stopManagedCampaign(db, WORKSPACE, made.campaign.id, NOW)).status).toBe(
      'stopped'
    );
    const actions = await db
      ...
      .get<{ total: number }>(WORKSPACE, made.campaign.id);
  ...
  it('discovers every seat with due work, each as its own seat', async () => {
    const mine = async () =>
      (await seatsWithDueActions(db, NOW)).filter((seat) => seat.workspaceId === WORKSPACE);
    await upsertSeat(
      db,
      WORKSPACE,
    ...
    await recordAction(
      db,
      {
        ...
        seatKey: 'secondary',
        kind: 'profile_view',
        targetRef: 'https://www.linkedin.com/in/secondary-test/',
        status: 'planned',
        source: 'export',
    ...
    await recordAction(
      db,
      {
        ...
        seatKey: 'owner',
        kind: 'profile_view',
        targetRef: 'https://www.linkedin.com/in/owner-test/',
        status: 'planned',
        source: 'export',
        plannedFor: NOW.toISOString()
      },
  ...
  it('uses the exact five-day 20/40/60/80/100 campaign ramp', () => {
    ...
    const fractions = [0, 1, 2, 3, 4, 9].map((days) =>
      campaignWarmupFraction(start, new Date(Date.parse(start) + days * 86_400_000))
    );
