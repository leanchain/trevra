...
export interface LinkedInActionRecord {
  ...
  /**
   * Approval/replay scope. Legacy rows use null and remain one-action-per-kind/
   * target. Manager workflow steps use a stable per-step hash, allowing a
   * later follow-up message without weakening idempotency for either step.
   */
...
export async function countActionsInWindow(
  db: Db, seat: SeatRef, kind: LinkedInActionKind, sinceHours: number, now: Date
): Promise<number> {
...
export async function dailyCountsForLastNDays(
  db: Db, seat: SeatRef, kind: LinkedInActionKind, n: number, now: Date
): Promise<number[]> {
  const days = Math.max(1, Math.trunc(n));
  const nowIso = now.toISOString();
  const since = new Date(now.getTime() - days * 86_400_000).toISOString();
  const rows = await db.prepare(`
    SELECT FLOOR(EXTRACT(EPOCH FROM (?::timestamptz - recorded_at)) / 86400)::int AS bucket,
           COUNT(*)::int AS total
    FROM linkedin_actions
...
export async function countPendingInvites(
  db: Db, seat: SeatRef, options: { before?: Date } = {}
): Promise<number> {
...
/**
 * Duplicate lookup. When payloadHash is undefined, preserve legacy semantics
 * and treat any earlier action of this kind/target as a duplicate. When an
 * explicit scope is supplied, only another row in that replay scope counts.
 */
export async function hasTarget(
  ...
  const scoped = payloadHash !== undefined;
  const row = await db.prepare(`
    SELECT id FROM linkedin_actions
    ...
    LIMIT 1
  `).get<{ id: string }>(seat.workspaceId, seat.seatKey, kind, targetRef, excludeActionId, scoped, payloadHash ?? null);
