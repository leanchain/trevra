...
import type { AutomationRule, ConnectionSummary, PreparedAction, Recommendation } from '../shared/types.js';

export function listRecommendations(db: Db, workspaceId: string): Recommendation[] {
  ...
  `);
  const proofStmt = db.prepare('SELECT id,summary,status FROM proof_packs WHERE recommendation_id=?');
  const proofItemsStmt = db.prepare(`
    SELECT id,source_type,source_id,label,category,external_url,excerpt
  ...
  return rows.map((row) => {
    const evidence = serializeEvidence(evidenceStmt.all(String(row.id)) as Array<Record<string, unknown>>);
    const proof = proofStmt.get(String(row.id)) as Record<string, unknown> | undefined;
    const action = actionStmt.get(String(row.id)) as Record<string, unknown> | undefined;
    return {
      id: String(row.id),
      ...
      proofPack: proof ? {
        ...
        items: serializeEvidence(proofItemsStmt.all(String(proof.id)) as Array<Record<string, unknown>>)
      } : null,
...
export function listConnections(db: Db, workspaceId: string): ConnectionSummary[] {
  const rows = db.prepare(`
...
export function listAutomationRules(db: Db, workspaceId: string): AutomationRule[] {
  const rows = db.prepare('SELECT * FROM automation_rules WHERE workspace_id=? ORDER BY recommendation_type').all(workspaceId) as Array<Record<string, unknown>>;
  return rows.map((row) => ({
    id: String(row.id),
