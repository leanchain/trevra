...
export async function listRecommendations(db: Db, workspaceId: string): Promise<Recommendation[]> {
  ...
  return Promise.all(rows.map(async (row) => {
    ...
    const proof = await db.prepare('SELECT id,summary,status FROM proof_packs WHERE recommendation_id=?')
...
export async function listAutomationRules(db: Db, workspaceId: string): Promise<AutomationRule[]> {
  const rows = await db.prepare('SELECT * FROM automation_rules WHERE workspace_id=? ORDER BY recommendation_type')
    .all<Record<string, unknown>>(workspaceId);
  return rows.map((row) => ({
    id: String(row.id),
