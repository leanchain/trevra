...
async function signUp(app: Express, label: string, name: string): Promise<SignedUp> {
  ...
  const res = await agent.post('/api/auth/sign-up/email').send({ email, password: 'correct horse battery staple', name }).expect(200);
...
describe('first sign-in (organization plugin routed, single-user flow unchanged)', () => {
  it('gives a brand-new email its own workspace, settings, default automation rules, audit event and marketing event', async () => {
    ...
    const settings = await database.prepare('SELECT sender_name,currency FROM workspace_settings WHERE workspace_id=?')
      .get<{ sender_name: string; currency: string }>(auth.workspaceId);
    expect(settings).toEqual({ sender_name: 'Solo Operator', currency: 'EUR' });

    const rules = await database.prepare('SELECT COUNT(*)::int AS n FROM automation_rules WHERE workspace_id=?')
      .get<{ n: number }>(auth.workspaceId);
    expect(rules?.n).toBe(4);

    const audit = await database.prepare("SELECT COUNT(*)::int AS n FROM audit_events WHERE workspace_id=? AND event_type='workspace.created'")
      .get<{ n: number }>(auth.workspaceId);
    expect(audit?.n).toBe(1);

    const marketing = await database.prepare("SELECT COUNT(*)::int AS n FROM marketing_events WHERE workspace_id=? AND event_name='signup_completed'")
      .get<{ n: number }>(auth.workspaceId);
    expect(marketing?.n).toBe(1);

    // The pinned-id architecture's whole point: the organization IS the workspace.
    const org = await database.prepare('SELECT id FROM organization WHERE id=?').get<{ id: string }>(auth.workspaceId);
    expect(org?.id).toBe(auth.workspaceId);
    const member = await database.prepare('SELECT role FROM member WHERE "organizationId"=? AND "userId"=(SELECT id FROM "user" WHERE lower(email)=?)')
...
describe('two users, one workspace', () => {
  it('a member added to another workspace sees that workspace\'s data through the normal request flow', async () => {
    ...
    await betterAuth.api.addMember({ body: { userId: teammate.userId, organizationId: ownerAuth.workspaceId, role: 'member' } });
    await teammate.agent.post('/api/auth/organization/set-active').send({ organizationId: ownerAuth.workspaceId }).expect(200);
    ...
    expect((teammateDashboard.body.workspace as { id: string }).id).toBe((ownerDashboard.body.workspace as { id: string }).id);
  });
});
...
describe('POST /api/team/members (add teammate: always a real invitation, must be accepted)', () => {
  ...
  it('is owner-only: a member operating in this workspace cannot add teammates to it', async () => {
    ...
    const attempt = await member.agent.post('/api/team/members').send({ email: uniqueEmail('irrelevant') });
...
describe('credential route gate', () => {
  ...
  it('403s a member and lets the owner through (200, or a deployment-fact 409 -- never 403)', async () => {
    ...
    const memberAttempt = await member.agent.post('/api/linkedin/seat/credentials').send({ email: 'seat@example.com', password: 'seat-password-123' });
    expect(memberAttempt.status).toBe(403);

    const ownerAttempt = await owner.agent.post('/api/linkedin/seat/credentials').send({ email: 'seat@example.com', password: 'seat-password-123' });
...
describe('backfill', () => {
  it('creates an organization + owner member for a pre-existing workspace, and is idempotent on a second run', async () => {
    ...
    const signedUp = await betterAuth.api.signUpEmail({ body: { email, password: 'correct horse battery staple', name: 'Backfill Owner' } });
    const workspaceId = id('ws');
    const userId = id('usr');
    const now = new Date().toISOString();
    await database.prepare('INSERT INTO workspaces (id,name,created_at) VALUES (?,?,?)').run(workspaceId, "Backfill Owner's Studio", now);
    await database.prepare('INSERT INTO users (id,workspace_id,email,name,created_at) VALUES (?,?,?,?,?)').run(userId, workspaceId, email, 'Backfill Owner', now);
    ...
    await database.prepare('INSERT INTO workspace_settings (workspace_id,currency,sender_name,timezone,demo_mode,created_at,updated_at) VALUES (?,?,?,?,?,?,?)')
    ...
    const org = await database.prepare('SELECT id,name FROM organization WHERE id=?').get<{ id: string; name: string }>(workspaceId);
    expect(org?.id).toBe(workspaceId);
    const members = await database.prepare('SELECT role FROM member WHERE "organizationId"=?').all<{ role: string }>(workspaceId);
    ...
    const settings = await database.prepare('SELECT sender_name FROM workspace_settings WHERE workspace_id=?').get<{ sender_name: string }>(workspaceId);
    expect(settings?.sender_name).toBe('Backfill Owner');

    const second = await backfillWorkspaceOrganizations(database);
    expect(second.created).toBe(0);

    const orgsAfterSecondRun = await database.prepare('SELECT COUNT(*)::int AS n FROM organization WHERE id=?').get<{ n: number }>(workspaceId);
    expect(orgsAfterSecondRun?.n).toBe(1);
    const membersAfterSecondRun = await database.prepare('SELECT COUNT(*)::int AS n FROM member WHERE "organizationId"=?').get<{ n: number }>(workspaceId);
    expect(membersAfterSecondRun?.n).toBe(1);

    // The workspace's own id never moved -- the whole point of pinning.
    const workspaceRow = await database.prepare('SELECT id FROM workspaces WHERE id=?').get<{ id: string }>(workspaceId);
  ...
  it('skips the seeded demo workspace', async () => {
    ...
    const demoOrg = await database.prepare('SELECT id FROM organization WHERE id=?').get<{ id: string }>(DEMO_WORKSPACE_ID);
...
describe('last-owner protection', () => {
  ...
  it('rejects demoting the sole owner away from the owner role', () => {
    const members = [{ userId: 'usr_a', role: 'owner' }, { userId: 'usr_b', role: 'member' }];
    expect(() => assertOwnerChangeAllowed(members, 'usr_a', false)).toThrow(/only owner/i);
  });

  it('allows removing one of several owners', () => {
    const members = [{ userId: 'usr_a', role: 'owner' }, { userId: 'usr_b', role: 'owner' }];
  ...
  it('is wired into better-auth\'s own remove-member route, not only reachable from a future bespoke one', async () => {
    ...
    const selfRemoval = await owner.agent.post('/api/auth/organization/remove-member')
    ...
    const stillMember = await database.prepare('SELECT role FROM member WHERE "organizationId"=? AND "userId"=(SELECT id FROM "user" WHERE lower(email)=?)')
