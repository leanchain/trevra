...
import { resolvePersonByIdentity } from '../person-identities.js';
...
/**
 * Map whatever we know to a canonical Person, and thereby to an email the CRM
 * can actually be queried with.
 *
 * A platform handle only ever resolves LOCALLY, through `person_identities`.
 ...
 * record.
 */
export async function resolveLocalContact(
  db: Db,
  workspaceId: string,
  hint: ContactHint
): Promise<ResolvedLocalContact> {
  const email = hint.email?.trim().toLowerCase() || null;

  if (email) {
    const person = await db
      .prepare('SELECT id,email FROM contacts WHERE workspace_id=? AND email_normalized=? LIMIT 1')
      .get<{ id: string; email: string | null }>(workspaceId, email);
    return {
      personId: person?.id ?? null,
      email: person?.email?.toLowerCase() ?? email
    };
  }

  const handle = hint.handle?.trim();
  if (handle && hint.handleProvider) {
    const person = await resolvePersonByIdentity(db, {
      workspaceId,
      provider: hint.handleProvider,
      identityType: 'handle',
...
export function nangoProxy(providerConfigKey: string, connectionId: string): CrmProxy {
  ...
  return {
    async post<T>(endpoint: string, data: unknown): Promise<T> {
      const response = await nango.post<T>({
        endpoint,
        providerConfigKey,
        connectionId,
        retries: 2,
    ...
    async get<T>(endpoint: string, params: Record<string, string> = {}): Promise<T> {
      const response = await nango.get<T>({
        endpoint,
        providerConfigKey,
        connectionId,
        retries: 2,
...
export async function logCrmActivity(
  ...
  `
    ...
    .get<{ id: string; status: string; external_ref: string | null }>(
      workspaceId,
      provider,
      payload.sourceType,
      payload.sourceId
  ...
  const proxy = (options.proxyFor ?? nangoProxy)(
  ...
  let contact;
  try {
    contact = await adapter.findContact(
      { ...payload.contact, email: local.email ?? payload.contact.email ?? null },
      proxy
    );
  } catch (cause) {
    // A lookup failure wrote nothing, so it is safe to report and move on.
    return skip(
      `${adapter.name} contact lookup failed: ${cause instanceof Error ? cause.message : String(cause)}`,
      provider
    );
  }
  ...
  if (!contact) {
    ...
    return {
      ...
      provider,
      externalRef: null,
      reason: 'No matching CRM contact; nothing was created.',
      activityId
    };
  }
  ...
  try {
    ...
    await db
      .prepare('UPDATE crm_activities SET status=?, external_ref=? WHERE id=?')
      .run('written', externalRef, activityId);
    ...
    await db
      .prepare('UPDATE crm_activities SET status=?, error=? WHERE id=?')
...
async function insertActivity(
  ...
  await db
    ...
    INSERT INTO crm_activities (
      id, workspace_id, provider, connection_id, person_id, contact_external_id,
