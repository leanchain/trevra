...
try {
  if (command === 'skills') {
    ...
    if (!args[0]) throw new Error('Usage: npm run agent -- playbook:start <playbook-id> <json-or-@file> [version]');
    print(await client.startPlaybook(args[0], await parseInput(args[1] ?? '{}'), args[2]));
  } else if (command === 'playbook:runs') {
    print(await client.listPlaybookRuns({ status: args[0] as never, limit: args[1] ? Number(args[1]) : undefined }));
    ...
    print(await client.listEvents({ streamType: args[0], streamId: args[1], limit: args[2] ? Number(args[2]) : undefined }));
  } else if (command === 'brief') {
    print(await client.getRevenueBrief());
  } else if (command === 'actions') {
    print(await client.listPendingActions());
  } else if (command === 'prepare') {
    if (!args[0]) throw new Error('Usage: npm run agent -- prepare <recommendation-id>');
    print(await client.prepareRecommendation(args[0]));
  } else if (command === 'run') {
    ...
    print(await client.listRuns({ skillId: args[0], limit: args[1] ? Number(args[1]) : undefined }));
...
async function parseInput(value: string): Promise<unknown> {
  ...
  catch { throw new Error('Skill input must be valid JSON or @path/to/input.json'); }
}
