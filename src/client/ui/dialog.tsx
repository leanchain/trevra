...
/**
 * Put a view in the URL, where it can be typed, bookmarked, shared and
 * Back-ed out of.
 *
 * The hash is read as a parameter list (`#view=setup&tab=safety`) rather than a
 * single value, so independent bits of view state can live in it side by side
 * without any of them knowing about the others. A value that is not in
 * `allowed` -- a stale link, a typo, an unrelated anchor like `#get-started` --
 * falls back rather than rendering nothing.
 *
 * Assigning `location.hash` is deliberate: it pushes a real history entry, so
 * Back returns to the previous view instead of leaving the app.
 */
export function useHashView<T extends string>(
  ...
  // Held in a ref so callers can pass array literals without re-arming the
  // listener on every render.
  const spec = useRef({ allowed, fallback });
  spec.current = { allowed, fallback };

  const read = useCallback((): T => {
    const raw = hashParams().get(key);
    const { allowed: valid, fallback: none } = spec.current;
    return raw !== null && (valid as readonly string[]).includes(raw) ? (raw as T) : none;
  }, [key]);
