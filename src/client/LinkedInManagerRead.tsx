...
const STATUS_ORDER: readonly MemberStatus[] = ['active', 'waiting', 'manual', 'pending', 'paused', 'replied', 'completed', 'removed', 'failed'];
...
function countByStatus(members: readonly ManagedCampaignMember[]): Record<MemberStatus, number> {
  const counts = {
    pending: 0, active: 0, waiting: 0, manual: 0, paused: 0, replied: 0, completed: 0, removed: 0, failed: 0
 ...
 */
function warmupOf(campaign: ManagedCampaign, now: number, report: LinkedInLimitsReport | null): { day: number; days: number; fraction: number } | null {
...
function StatusBar({ counts, total }: { counts: Record<MemberStatus, number>; total: number }) {
  ...
  return <svg className="mgr-bar" viewBox="0 0 100 6" preserveAspectRatio="none" aria-hidden="true" focusable="false">
    {segments.map((segment) => <rect key={segment.status} className={`mgr-seg-${segment.status}`} x={segment.x} y="0" width={segment.width} height="6" />)}
  </svg>;
}
...
function StatusLegend({ counts, total }: { counts: Record<MemberStatus, number>; total: number }) {
  ...
  return <ul className="mgr-legend">
    {STATUS_ORDER.filter((status) => counts[status] > 0).map((status) => <li key={status}>
      <i className={`mgr-dot-${status}`} aria-hidden="true" /><b>{counts[status]}</b> {STATUS_LABEL[status].toLowerCase()}
    </li>)}
  </ul>;
}

function MemberState({ status }: { status: MemberStatus }) {
  return <span className="mgr-state"><i className={`mgr-dot-${status}`} aria-hidden="true" />{STATUS_LABEL[status]}</span>;
}

/** The ramp's days, as pips. No dynamic width, so no inline style. */
function WarmupPips({ day, days }: { day: number; days: number }) {
  return <span className="mgr-pips" aria-hidden="true">
    {Array.from({ length: days }, (_, index) => index + 1).map((pip) => <i key={pip} className={pip <= day ? 'is-on' : ''} />)}
  </span>;
}
 ...
 */
function Allowance({ ceilings, of }: { ceilings: Record<ManagedKind, EnforcedCeiling>; of: 'today' | 'full' }) {
  return <>: {plural(ceilings.invite[of], 'invite')}, {plural(ceilings.dm[of], 'message')}, {plural(ceilings.profile_view[of], 'profile view')} and {plural(ceilings.follow[of], 'follow')}</>;
}

function stepCopy(step: WorkflowStep, variantId: string | null): { body: string | null; variant: string | null; note: string | null } {
  if (step.action === 'message') {
    const variants = step.config.variants;
    const chosen = variantId ? variants.find((variant) => variant.id === variantId) ?? null : null;
    if (chosen) return { body: chosen.body, variant: chosen.id, note: variants.length > 1 ? `Version ${chosen.id} of ${variants.length} in test` : null };
    return { body: variants[0]?.body ?? null, variant: null, note: variants.length > 1 ? `${variants.length} versions in test — one is picked for this lead when the step runs` : null };
  }
  if (step.action === 'connection_request') {
    return { body: step.config.message ?? null, variant: null, note: step.config.message ? null : 'Sent without a note.' };
  }
  if (step.action === 'manual_message') {
    return { body: step.config.suggestedTemplate ?? null, variant: null, note: 'You write and send this one yourself.' };
  }
  if (step.action === 'withdraw_pending') {
    return { body: null, variant: null, note: `Only if the invite is still pending after ${plural(step.config.afterDays, 'day')}.` };
  }
  ...
  if (step.action === 'profile_view') {
    return { body: null, variant: null, note: 'Nothing is sent. Their profile is opened, and the visit shows up on it.' };
  }
  if (step.action === 'follow') {
    return { body: null, variant: null, note: 'Nothing is sent. The account follows them, which LinkedIn may notify them about.' };
  }
  return { body: null, variant: null, note: null };
}

/** Where this one person is in the sequence, and what happens to them next. */
function MemberTimeline({ steps, member, now }: { steps: readonly WorkflowStep[]; member: ManagedCampaignMember; now: number }) {
  if (steps.length === 0) return <p className="empty-copy">This campaign&rsquo;s workflow has been deleted, so its steps can no longer be shown.</p>;
  let elapsed = 0;
  return <ol className="mgr-timeline">
    {steps.map((step, index) => {
      ...
      return <li key={step.id} className={`mgr-tl ${done ? 'mgr-tl-done' : current ? 'mgr-tl-current' : 'mgr-tl-next'}`}>
        ...
        <div>
          ...
          <p className="mgr-tl-when">
            ...
            {current && member.status === 'failed' && 'This step failed and the lead stopped here'}
            {current && (member.status === 'pending' || member.status === 'active' || member.status === 'waiting') && `Next up — ${dueIn(member.nextEligibleAt, now)}`}
            {current && (member.status === 'replied' || member.status === 'completed' || member.status === 'removed') && `Stopped here (${STATUS_LABEL[member.status].toLowerCase()})`}
            {!done && !current && 'Later'}
          </p>
...
function CampaignMembers({ members, steps, now, busy, onPause, onResume, onRemove }: {
  ...
  useEffect(() => { setLimit(50); }, [query, status, sort]);
  ...
  const shown = useMemo(() => {
    ...
    const filtered = members.filter((member) => {
      ...
      return `${member.firstName} ${member.lastName} ${member.company}`.toLowerCase().includes(needle);
    });
    ...
    return [...filtered].sort((a, b) => {
      ...
      if (sort === 'status') return STATUS_ORDER.indexOf(a.status) - STATUS_ORDER.indexOf(b.status) || byName(a, b);
  ...
  return <div className="mgr-members">
    <div className="li-filter-row">
      <label>Search leads<input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Name or company" type="search" /></label>
      <label>Status<select value={status} onChange={(event) => setStatus(event.target.value as 'all' | MemberStatus)}>
        <option value="all">All statuses</option>
        {present.map((value) => <option key={value} value={value}>{STATUS_LABEL[value]}</option>)}
      </select></label>
      <label>Sort by<select value={sort} onChange={(event) => setSort(event.target.value as MemberSort)}>
        <option value="next">Next action first</option>
        <option value="name">Name A–Z</option>
        <option value="status">Status</option>
        <option value="step">Furthest through the sequence</option>
      </select></label>
      <p className="li-hint">Showing {shown.length} of {plural(members.length, 'lead')}.</p>
    </div>

    {shown.length === 0
      ? <p className="empty-copy">No lead matches that search. Clear the filters to see the whole list.</p>
      : <div className="mgr-wide"><div className="li-table-scroll"><table className="li-table">
        <thead><tr><th>Lead</th><th>Company</th><th>Status</th><th>Step</th><th>Next action</th><th><span className="mgr-sr">Controls</span></th></tr></thead>
        <tbody>
          {shown.slice(0, limit).map((member) => {
            ...
            return [
              <tr key={member.id} className={open ? 'mgr-row-open' : undefined}>
                <td>
                  <button className="mgr-linkish" type="button" aria-expanded={open} onClick={() => setOpenId(open ? '' : member.id)}>
                    {open ? <ChevronDown size={14} /> : <ChevronRight size={14} />}
                    {member.firstName} {member.lastName}
                  </button>
                </td>
                <td>{member.company || '—'}</td>
                <td><MemberState status={member.status} /></td>
                <td className="li-num">{steps.length > 0 ? `${Math.min(member.stepIndex + 1, steps.length)} of ${steps.length}` : member.stepIndex + 1}</td>
                <td>{LIVE_STATUSES.includes(member.status) ? dueIn(member.nextEligibleAt, now) : '—'}</td>
                <td>
                  <div className="li-row-actions">
                    {LIVE_STATUSES.includes(member.status) && <button className="li-mini-button" type="button" disabled={busy !== ''} onClick={() => onPause(member)}>
                      {rowBusy ? <LoaderCircle className="spin" size={12} /> : <Pause size={12} />} Pause
                    </button>}
                    {member.status === 'paused' && <button className="li-mini-button" type="button" disabled={busy !== ''} onClick={() => onResume(member)}>
                      {rowBusy ? <LoaderCircle className="spin" size={12} /> : <Play size={12} />} Resume
                    </button>}
                    {(LIVE_STATUSES.includes(member.status) || member.status === 'paused') && <button className="li-mini-button li-mini-danger" type="button" disabled={busy !== ''} onClick={() => onRemove(member)}>
                      <Trash2 size={12} /> Remove
                    </button>}
                  </div>
                </td>
              </tr>,
              open ? <tr key={`${member.id}:detail`} className="mgr-row-detail"><td colSpan={6}>
    ...
    {shown.length > limit && <div className="mgr-more">
      <button className="secondary-button" type="button" onClick={() => setLimit((current) => current + 100)}>Show 100 more</button>
...
function VariantResults({ analytics, stepsById }: { analytics: ManagedAnalytics; stepsById: ReadonlyMap<string, WorkflowStep | null> }) {
  ...
  return <div className="mgr-ab">
    {groups.map(([stepId, rows]) => {
      ...
      const withRate = rows.map((row) => ({ ...row, rate: row.sent === 0 ? null : row.replied / row.sent }));
      const thin = withRate.filter((row) => row.sent < MIN_VARIANT_SENDS);
      const best = [...withRate].sort((a, b) => (b.rate ?? -1) - (a.rate ?? -1))[0];
      const decided = thin.length === 0 && withRate.length > 1 && withRate.some((row) => (row.rate ?? 0) !== (best.rate ?? 0));
      const peak = Math.max(...withRate.map((row) => row.rate ?? 0), 0.0001);
      return <div className="mgr-ab-group" key={stepId}>
        <h5 className="mgr-ab-head" aria-level={4} role="heading">{step ? ACTION_LABEL[step.action] : 'A message step'}<span>{step
          ? `Step "${stepId}"`
          : known
            ? `Step "${stepId}" — more than one campaign here runs a different step under that name`
            : `Step "${stepId}" — no campaign in this filter still runs it`}</span></h5>
        <div className="mgr-variants">
          {withRate.map((row) => {
            const body = step && step.action === 'message' ? step.config.variants.find((variant) => variant.id === row.variantId)?.body ?? null : null;
            const leading = decided && row.variantId === best.variantId;
            return <article className={`mgr-variant${leading ? ' is-leading' : ''}`} key={row.variantId}>
              <header>
...
export function OutreachManagerRead({ setToast }: { setToast: (message: string) => void }) {
  ...
  const [membersByCampaign, setMembersByCampaign] = useState<Record<string, ManagedCampaignMember[]>>({});
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const details = await Promise.all(nextCampaigns.map(async (campaign) => {
        try { return [campaign.id, (await getLinkedInManagedCampaign(campaign.id)).members] as const; }
        catch { return [campaign.id, [] as ManagedCampaignMember[]] as const; }
      }));
      ...
      const reports = await Promise.all(nextSeats.map(async (seat) => {
        try { return [seat.seatKey, await getLinkedInLimits(seat.seatKey)] as const; }
        catch { return [seat.seatKey, null] as const; }
      }));
      setLimitsBySeat(Object.fromEntries(
        reports.filter((entry): entry is readonly [string, LinkedInLimitsReport] => entry[1] !== null)
      ));
      setNow(Date.now());
      setError('');
    } catch (err) {
  ...
  const loadAnalytics = useCallback(async () => {
    ...
    try {
      setAnalytics(await getLinkedInManagedAnalytics({
  ...
  const refreshAll = useCallback(async () => { await Promise.all([load(), loadAnalytics()]); }, [load, loadAnalytics]);
  useOutreachRefresh(refreshAll);

  const seatLabel = useCallback((seatKey: string) => seats.find((seat) => seat.seatKey === seatKey)?.label ?? seatKey, [seats]);
  const workflowOf = useCallback((campaign: ManagedCampaign) => workflows.find((workflow) => workflow.id === campaign.workflowId) ?? null, [workflows]);
  const listOf = useCallback((campaign: ManagedCampaign) => lists.find((list) => list.id === campaign.leadListId) ?? null, [lists]);
  ...
  const refreshCampaign = async (campaignId: string) => {
    ...
    setCampaigns((current) => current.map((campaign) => campaign.id === campaignId ? detail.campaign : campaign));
  };
  ...
  const startCampaign = (campaign: ManagedCampaign) => guard(`campaign:${campaign.id}`, async () => {
    await startLinkedInManagedCampaign(campaign.id);
    setToast(`“${campaign.name}” is running. First actions are queued for the account's next working hours.`);
    await refreshAll();
  }, 'Unable to start that campaign.');

  const pauseCampaign = (campaign: ManagedCampaign) => guard(`campaign:${campaign.id}`, async () => {
  ...
  const stopCampaign = (campaign: ManagedCampaign) => guard(`campaign:${campaign.id}`, async () => {
    await stopLinkedInManagedCampaign(campaign.id);
    setToast(campaign.status === 'draft'
  ...
  const setMemberPaused = (member: ManagedCampaignMember, paused: boolean) => guard(`member:${member.id}`, async () => {
    await setLinkedInManagedMemberPaused(member.id, paused);
    setToast(`${member.firstName} ${member.lastName} ${paused ? 'paused' : 'resumed'} in this campaign.`);
    await refreshCampaign(member.campaignId);
  }, paused ? 'Unable to pause that lead.' : 'Unable to resume that lead.');

  const removeMember = (member: ManagedCampaignMember) => guard(`member:${member.id}`, async () => {
    await removeLinkedInManagedMember(member.id);
    setToast(`${member.firstName} ${member.lastName} removed. They can be added to another campaign now.`);
    await refreshCampaign(member.campaignId);
  }, 'Unable to remove that lead.');

  const completeTask = (task: ManualTaskView) => guard(`task:${task.id}`, async () => {
    await completeLinkedInManualTask(task.id);
    setToast(`Marked done for ${task.firstName} ${task.lastName}. The sequence continues from the next step.`);
    await refreshAll();
  ...
  const copyBody = async (task: ManualTaskView) => {
    ...
    } catch { setError('Your browser blocked the copy. Select the text and copy it by hand.'); }
  };

  const runNow = () => guard('tick', async () => {
  ...
  const variantStepsById = useMemo(() => {
    const scope = campaignFilter ? campaigns.filter((campaign) => campaign.id === campaignFilter) : campaigns;
    ...
    for (const campaign of scope) {
      for (const step of campaign.steps) {
        ...
        if (seen === undefined) { shapes.set(step.id, shape); byId.set(step.id, step); }
  ...
  return <div className="page-stack">
    ...
    <section className="page-panel" id="mgr-campaigns">
      <div className="section-heading">
        ...
        <div className="mgr-actions">
          ...
          <button className="secondary-button" type="button" disabled={loading} onClick={() => void refreshAll()}>
      ...
      <p className="mgr-summary">
        ...
        <span>{plural(pendingTasks.length, 'message')} waiting for you</span>
      </p>
      {runningCount === 0 && campaigns.length > 0 && <p className="li-hint">Run now becomes available once a campaign is running.</p>}

      {tick && <div className="li-dryrun">
        <Zap size={18} />
        <div>
          <strong>Run now finished</strong>
          <p>
            Advanced {plural(tick.campaignsTicked, 'campaign')}: {plural(tick.actionsPlanned, 'action')} queued,
            {' '}{plural(tick.manualTasksCreated, 'message')} for you to write, {plural(tick.membersCompleted, 'lead')} finished.
            {tick.membersBlocked > 0 && ` ${plural(tick.membersBlocked, 'lead')} could not move yet — no profile link, or today's limit for that account is already used.`}
          </p>
        </div>
      </div>}
        ...
        : campaigns.length === 0
          ...
          : <div className="mgr-list">
            {campaigns.map((campaign) => {
               ...
               */
              const campaignSteps = campaign.steps.length > 0 ? campaign.steps : workflow?.steps ?? [];
              const ceilings = enforcedCeilings(report, warmup?.fraction ?? rampFractionForDay(report, 1) ?? 1);
              ...
              return <article className={`mgr-campaign${open ? ' is-open' : ''}`} key={campaign.id}>
                <div className="mgr-campaign-head">
                  <button className="mgr-toggle" type="button" aria-expanded={open} onClick={() => setOpenCampaignId(open ? '' : campaign.id)}>
                    {open ? <ChevronDown size={16} /> : <ChevronRight size={16} />}
                    <strong>{campaign.name}</strong>
                    <span className={`li-chip mgr-chip-${campaign.status}`}>{CAMPAIGN_STATUS_LABEL[campaign.status]}</span>
                  </button>
                  <div className="mgr-actions">
                    {(campaign.status === 'draft' || campaign.status === 'paused') && <button className="primary-button" type="button" disabled={busy !== ''} onClick={() => void startCampaign(campaign)}>
                      {busyHere ? <LoaderCircle className="spin" size={14} /> : <Play size={14} />} {campaign.status === 'draft' ? 'Start' : 'Resume'}
                    </button>}
                    {campaign.status === 'running' && <button className="secondary-button" type="button" disabled={busy !== ''} onClick={() => void pauseCampaign(campaign)}>
                      {busyHere ? <LoaderCircle className="spin" size={14} /> : <Pause size={14} />} Pause
                    </button>}
                    {/*
                      ...
                      it happens.
                    */}
                    {(campaign.status === 'draft' || campaign.status === 'running' || campaign.status === 'paused') && <button className="secondary-button" type="button" disabled={busy !== ''} onClick={() => setStopping(campaign)}>
                      <Square size={14} /> {campaign.status === 'draft' ? 'Cancel' : 'Stop'}
                    </button>}
                    {terminal && rebuildable && <button className="secondary-button" type="button" disabled={busy !== ''} onClick={() => rebuild(campaign)}>
                ...
                <p className="mgr-meta">
                  <span>Sends from <b>{seatLabel(campaign.seatKey)}</b></span>
                  <span>{plural(campaign.memberCount, 'lead')} from {listOf(campaign)?.name ?? 'a deleted list'}</span>
                  <span>{workflow
                    ? `${workflow.name} · ${plural(campaignSteps.length, 'step')}${campaign.workflowVersion === null
                      ? ''
                ...
                */}
                {terminal
                  ? <p className="mgr-warmup">{campaign.status === 'stopped'
                    ? <>Stopped. Every lead still in it was taken out where they stood and any queued action that had not started was cancelled, so those leads are free for another campaign. A stopped campaign cannot be started again{rebuildable ? <> — build a new one from the same list and workflow to pick the work back up.</> : ', and its lead list or workflow no longer exists to rebuild it from.'}</>
                    : <>Finished: every lead reached the end of the workflow, and nothing more goes out from it{rebuildable ? <> — build a new one from the same list and workflow to run them through it again.</> : '.'}</>}</p>
                  : <p className="mgr-warmup">
                    {campaign.status === 'draft'
                      ? <>Not started. {rampFractions(report) && fullCeilings
                        ? <>Day 1 is held to {Math.round((rampFractionForDay(report, 1) ?? 1) * 100)}% of what {seatLabel(campaign.seatKey)} may send, reaching full speed on day {rampFractions(report)?.length}. At full speed that is {plural(fullCeilings.invite.full, 'invite')} and {plural(fullCeilings.dm.full, 'message')} a day.</>
                        : <>Nothing goes out until you start it.</>}</>
                      : warmup && warmup.fraction < 1
                        ? <><WarmupPips day={warmup.day} days={warmup.days} /> Warm-up day {warmup.day} of {warmup.days} — this campaign may use {Math.round(warmup.fraction * 100)}% of what {seatLabel(campaign.seatKey)} is allowed today{ceilings ? <Allowance ceilings={ceilings} of="today" /> : ''}. That is why day one looks slow.{ceilings && <> The invite ceiling is {ceilingSourceNote(ceilings.invite)}.</>}</>
                        : warmup
                          ? <><WarmupPips day={warmup.days} days={warmup.days} /> Full speed{ceilings ? <Allowance ceilings={ceilings} of="full" /> : ''}.{ceilings && <> The invite ceiling is {ceilingSourceNote(ceilings.invite)}.</>}</>
                          : campaign.startedAt
                            ...
                            ? <>Running. Today&rsquo;s allowance for {seatLabel(campaign.seatKey)} could not be read just now.</>
                            : <>Warm-up starts when the campaign starts.</>}
                  </p>}

                {open && <CampaignMembers
                  members={members}
                  steps={campaignSteps}
                  now={now}
                  busy={busy}
    ...
    <section className="page-panel">
      ...
      <div className="li-filter-row">
        <label>Campaign<select value={campaignFilter} onChange={(event) => setCampaignFilter(event.target.value)}>
          <option value="">All campaigns</option>
          {campaigns.map((campaign) => <option key={campaign.id} value={campaign.id}>{campaign.name}</option>)}
        </select></label>
        <label>LinkedIn account<select value={seatFilter} onChange={(event) => pickSeatFilter(event.target.value)}>
        ...
        <div className="mgr-window" role="group" aria-label="Time window">
          {([['7 days', 7], ['30 days', 30], ['90 days', 90], ['All time', null]] as const).map(([label, value]) => <button
            key={label}
        ...
        {analyticsLoading && <span className="li-hint"><LoaderCircle className="spin" size={12} /> Reading…</span>}
      </div>
      ...
      <div className="li-stat-row mgr-stats">
        ...
        */}
        <div className="li-stat"><p>Follows</p><strong>{analytics?.followsSent ?? 0}</strong></div>
        <div className="li-stat"><p>Invites withdrawn</p><strong>{analytics?.invitesWithdrawn ?? 0}</strong><span>Pending invites a workflow step cancelled</span></div>
      </div>

      <h4 className="li-subhead" aria-level={3}>Message versions, side by side</h4>
      {analytics ? <VariantResults analytics={analytics} stepsById={variantStepsById} /> : <p className="empty-copy">Reading results…</p>}
    </section>
    ...
    <section className="page-panel">
      ...
      {tasks.length === 0
        ...
        : <div className="mgr-wide"><div className="li-table-scroll"><table className="li-table">
          <thead><tr><th>Lead</th><th>Company</th><th>Campaign</th><th>Account</th><th>Waiting</th><th><span className="mgr-sr">Controls</span></th></tr></thead>
          <tbody>
            {[...tasks].sort((a, b) => Number(b.status === 'pending') - Number(a.status === 'pending')).map((task) => {
              const open = openTaskId === task.id;
              return [
                <tr key={task.id} className={open ? 'mgr-row-open' : undefined}>
                  <td>
                    <button className="mgr-linkish" type="button" aria-expanded={open} onClick={() => setOpenTaskId(open ? '' : task.id)}>
                      {open ? <ChevronDown size={14} /> : <ChevronRight size={14} />}
                      {task.firstName} {task.lastName}
                    </button>
                  </td>
                  <td>{task.company || '—'}</td>
                  <td>{campaigns.find((campaign) => campaign.id === task.campaignId)?.name ?? 'Deleted campaign'}</td>
                  ...
                  */}
                  ...
                  <td>
                    {task.status === 'pending' && <div className="li-row-actions">
                      <button className="li-mini-button" type="button" disabled={busy !== ''} onClick={() => void completeTask(task)}>
                        {busy === `task:${task.id}` ? <LoaderCircle className="spin" size={12} /> : <Check size={12} />} Mark done
                      </button>
                    </div>}
                  </td>
                </tr>,
                open ? <tr key={`${task.id}:detail`} className="mgr-row-detail"><td colSpan={6}>
                  {task.status === 'cancelled' && <p className="mgr-tl-note">Cancelled: its campaign was stopped, or this lead was taken out of it. Nothing needs sending, and the sequence will not carry on from here.</p>}
                  {task.suggestedBody
    ...
    <section className="page-panel">
      ...
      {seats.length === 0
        ...
        : <div className="mgr-wide"><div className="li-table-scroll"><table className="li-table">
          ...
          */}
          <thead><tr><th>Account</th><th>Timezone</th><th>Working days</th><th>Hours</th><th>Your setting</th><th>Trevra&rsquo;s band</th><th>What actually goes out</th></tr></thead>
          <tbody>{seats.map((seat) => {
            const ceilings = enforcedCeilings(limitsBySeat[seat.seatKey] ?? null, 1);
            return <tr key={seat.seatKey}>
              <td><strong>{seat.label}</strong>{seat.seatKey === activeSeatKey && <> <span className="li-chip">Active</span></>}</td>
              <td>{seat.timezone}</td>
              <td>{seat.workingDays.length === 0 ? <span className="mgr-state"><i className="mgr-dot-failed" aria-hidden="true" />None — this account cannot work</span> : seat.workingDays.map((day) => WEEKDAYS[day]).join(', ')}</td>
              <td>{clock(seat.workStartMinute)}–{clock(seat.workEndMinute)}</td>
              <td>{seat.dailyInviteLimit} · {seat.dailyMessageLimit} · {seat.dailyProfileViewLimit} · {seat.dailyFollowLimit}</td>
              <td>{ceilings ? `${ceilings.invite.band} · ${ceilings.dm.band} · ${ceilings.profile_view.band} · ${ceilings.follow.band}` : '—'}</td>
              <td>{ceilings
                ? <>{ceilings.invite.full} · {ceilings.dm.full} · {ceilings.profile_view.full} · {ceilings.follow.full}{ceilings.invite.source === 'operator-override' && <> <span className="li-chip">Your numbers</span></>}</>
                : '—'}</td>
            </tr>;
          })}</tbody>
        </table></div></div>}
    ...
    <section className="page-panel">
      <div className="section-heading"><div>
        ...
        <p>Names, companies and contact details, kept with a note of where each list came from.</p>
      </div></div>
      {lists.length === 0
        ? <p className="empty-copy">No lead list yet. Import a CSV above and it becomes available to every campaign.</p>
        : <div className="mgr-wide"><div className="li-table-scroll"><table className="li-table">
          <thead><tr><th>List</th><th>Source</th><th>Leads</th><th>Updated</th></tr></thead>
          <tbody>{lists.map((list) => <tr key={list.id}>
    ...
    <section className="page-panel">
      <div className="section-heading"><div>
        <h3 aria-level={2}><WorkflowIcon size={18} className="li-heading-icon" /> Saved workflows</h3>
        <p>A workflow is the order of steps and the wait between them. A campaign runs the copy of the workflow it was started with, so editing one here shapes the campaigns you start from now on and leaves the ones already going on their own copy — nobody mid-sequence is moved to a different step. To put an edit in front of a campaign that is already running, pause it and start it again.</p>
      </div></div>
      {workflows.length === 0
        ? <p className="empty-copy">No workflow yet. Build one above — a view, an invite and two follow-up messages is the usual shape.</p>
        : <div className="mgr-wide"><div className="li-table-scroll"><table className="li-table">
          <thead><tr><th>Workflow</th><th>Steps</th><th>Takes</th><th>Updated</th></tr></thead>
          <tbody>{workflows.map((workflow) => {
            const hours = workflow.steps.reduce((total, step) => total + stepHours(step), 0);
            return <tr key={workflow.id}>
              <td><strong>{workflow.name}</strong></td>
              <td className="li-num">{workflow.steps.length}</td>
              <td>{hours < 24 ? 'Same day' : `about ${plural(Math.ceil(hours / 24), 'day')}`}</td>
    ...
    {stopping && <ConfirmDrawer
      ...
      body={stopping.status === 'draft'
        ? <>
          <p>This campaign has never run, so nothing has gone out and nothing is queued. Cancelling takes its leads back out of it, which frees them for another campaign.</p>
          <p>There is no way to delete a campaign, so it stays in the list as stopped, and a stopped campaign cannot be started. Build a new one when you want this list to run.</p>
        </>
        : <>
          <p>Every lead still in this campaign stops where they are, and any queued action that has not started is cancelled.</p>
          <p>A stopped campaign cannot be started again — you would create a new one. To pick it up later, pause it instead.</p>
        </>}
