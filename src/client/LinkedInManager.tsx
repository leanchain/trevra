  ...
  return <div style={{ display: 'grid', gap: 16 }}>
    <div style={card}>
      ...
      <div style={{ display: 'grid', gap: 10, marginTop: 14 }}>{steps.map((step, index) => <div key={`${step.id}-${index}`} style={card}>
        ...
        {step.kind === 'invite' && <label style={{ ...field, marginTop: 10 }}><span style={label}>Invite note</span><textarea style={input} rows={3} value={String(step.config.note ?? '')} onChange={(e) => setStep(index, { ...step, config: { note: e.target.value } })} /></label>}
        {step.kind === 'withdraw' && <label style={{ ...field, marginTop: 10 }}><span style={label}>Withdraw if still pending after days</span><input style={input} type="number" min={1} value={Number(step.config.olderThanDays ?? 14)} onChange={(e) => setStep(index, { ...step, config: { olderThanDays: Number(e.target.value) } })} /></label>}
        {step.kind === 'message' && <div style={{ marginTop: 10 }}><label style={field}><span style={label}>Variant A message</span><textarea style={input} rows={4} value={String((step.config.variants as Array<{ body?: string }> | undefined)?.[0]?.body ?? '')} onChange={(e) => setStep(index, { ...step, config: { variants: [{ id: 'A', body: e.target.value, weight: 100 }] } })} /></label><small style={{ color: 'var(--text-muted)' }}>A/B variants can be expanded in the stored workflow JSON; the server preserves deterministic weights.</small></div>}
        {step.kind === 'manual_message' && <label style={{ ...field, marginTop: 10 }}><span style={label}>Manual task instructions</span><textarea style={input} rows={3} value={String(step.config.instructions ?? '')} onChange={(e) => setStep(index, { ...step, config: { instructions: e.target.value } })} /></label>}
      </div>)}</div>
