...
import type { AgentRun, AgentRunStep, PlaybookRun, PlaybookStepRun, SkillRun } from '../../shared/types';
...
const KEY_ACRONYMS: Record<string, string> = {
  api: 'API', cta: 'CTA', crm: 'CRM', csv: 'CSV', eur: 'EUR', gbp: 'GBP',
  html: 'HTML', id: 'ID', ids: 'IDs', roi: 'ROI', url: 'URL', urls: 'URLs', usd: 'USD', vat: 'VAT'
};

export function humanizeKey(key: string): string {
  const spaced = key.replace(/([a-z0-9])([A-Z])/g, '$1 $2').replace(/[_-]+/g, ' ').trim();
  if (!spaced) return key;
  const words = spaced.split(/\s+/).map((word) => KEY_ACRONYMS[word.toLowerCase()] ?? word.toLowerCase());
 ...
 * Schema-driven forms.
 *
 * Every playbook and every skill already publishes its input schema -- the
 * server generates it from the same Zod definition the engine validates
 * against. So the launcher does not need a hand-written form per playbook, and
 * a user does not need to hand-write JSON. One renderer, always in step with
 * the contract.
 *
 * State is keyed by dotted path (`lead.contactEmail`) and reassembled on
 * submit. Empty optional fields are dropped rather than sent as "", which the
 * schema would reject.
 * -------------------------------------------------------------------------- */
...
/** Unwrap the `anyOf: [T, null]` shape zod-to-json-schema emits for nullish fields. */
function effectiveNode(node: SchemaNode): SchemaNode {
  if (!node.anyOf) return node;
  const real = node.anyOf.find((entry) => entry.type !== 'null');
  return real ? { ...real, description: node.description ?? real.description } : node;
}
...
/** Turn the flat path map back into the nested object the API expects. */
export function buildSchemaValue(schema: SchemaNode, values: Record<string, string>): Record<string, unknown> {
  const result: Record<string, unknown> = {};
  const walk = (node: SchemaNode, prefix: string[]): void => {
    const current = effectiveNode(node);
    if (current.type === 'object' && current.properties) {
      for (const [key, child] of Object.entries(current.properties)) walk(child, [...prefix, key]);
      return;
    }
    ...
    if (current.type === 'number' || current.type === 'integer') {
      ...
      if (Number.isFinite(parsed)) setPath(result, prefix, parsed);
      return;
    }
    if (current.type === 'boolean') { setPath(result, prefix, raw === 'true'); return; }
    if (current.type === 'array') {
      const parts = raw.split(',').map((part) => part.trim()).filter(Boolean);
      if (parts.length > 0) setPath(result, prefix, parts);
      return;
    }
    setPath(result, prefix, raw);
  };
  walk(schema, []);
  return result;
}

function SchemaField({ name, node, path, required, values, readOnly, onChange }: {
  name: string;
  node: SchemaNode;
  path: string;
  required: boolean;
  ...
  const current = effectiveNode(node);
  const label = current.title ?? humanizeKey(name);
  ...
  if (current.type === 'object' && current.properties) {
    return (
      <fieldset className="schema-group">
        <legend>{label}</legend>
        {Object.entries(current.properties).map(([key, child]) => (
          <SchemaField
            key={key}
            name={key}
            node={child}
  ...
  const control = current.enum
    ? <select value={value} disabled={readOnly} onChange={(event) => onChange(path, event.target.value)}>
        <option value="">Choose…</option>
        {current.enum.map((option) => <option key={String(option)} value={String(option)}>{String(option)}</option>)}
      </select>
    : current.type === 'boolean'
      ...
      : <input
          type={current.type === 'number' || current.type === 'integer' ? 'number' : current.format === 'email' ? 'email' : 'text'}
          value={value}
          required={required}
          disabled={readOnly}
          placeholder={current.type === 'array' ? 'Comma separated' : ''}
          {...(current.minimum === undefined ? {} : { min: current.minimum })}
          {...(current.maximum === undefined ? {} : { max: current.maximum })}
...
export function SchemaForm({ schema, values, readOnly, emptyCopy, onChange }: {
  schema: SchemaNode;
  values: Record<string, string>;
  /** True on the skills screen, where the same renderer shows a contract rather than taking one. */
  readOnly?: boolean;
  emptyCopy?: string;
  onChange: (path: string, value: string) => void;
}) {
  const root = effectiveNode(schema);
  if (!root.properties) return <p className="schema-empty">{emptyCopy ?? 'This playbook takes no input. Just start it.'}</p>;
  return (
    <div className="schema-form">
      {Object.entries(root.properties).map(([key, child]) => (
        <SchemaField
          key={key}
          name={key}
          node={child}
...
/** Seed the form from the schema's own defaults so it opens usable, not blank. */
export function defaultsFor(schema: SchemaNode): Record<string, string> {
  ...
  const walk = (node: SchemaNode, prefix: string[]): void => {
    ...
    if (current.default !== undefined) seeded[prefix.join('.')] = String(current.default);
  };
  walk(schema, []);
  return seeded;
}
...
export function durationBetween(startedAt: string | null, finishedAt: string | null): number | null {
...
function playbookNode(step: PlaybookStepRun): InspectorNode {
  ...
  const facts: RunFact[] = [];
  if (step.skillId) facts.push({ label: 'What ran', value: `${step.skillId}${step.skillVersion ? ` v${step.skillVersion}` : ''}` });
  if (step.attempt > 1) facts.push({ label: 'Attempts', value: `${step.attempt} tries` });
  return {
    key: step.id,
    title: humanizeId(step.stepId),
    kindLabel: isApproval ? 'Your decision' : step.stepType === 'action' ? 'Does something outside Trevra' : 'Work step',
    status: step.status,
...
export function inspectorFromPlaybookRun(run: PlaybookRun): InspectorRun {
  ...
  return {
    ...
    facts: [
      { label: 'Started by', value: run.actorType === 'agent' ? 'Your agent' : run.actorType === 'user' ? 'You' : humanizeKey(run.actorType) },
...
export function inspectorFromSkillRun(run: SkillRun): InspectorRun {
  const durationMs = Number.isFinite(run.durationMs) && run.durationMs > 0
    ? run.durationMs
...
function FieldValue({ value, depth }: { value: unknown; depth: number }) {
  ...
  if (typeof value === 'number') return <span>{Number.isInteger(value) ? value.toLocaleString('en-US') : String(value)}</span>;
  if (typeof value === 'string') {
    const trimmed = value.trim();
    if (!trimmed) return <span className="field-muted">Empty</span>;
    if (/^https?:\/\/\S+$/.test(trimmed)) return <a href={trimmed} target="_blank" rel="noreferrer">{trimmed}</a>;
  ...
  if (Array.isArray(value)) {
    ...
    return flat
      ? <ul className="field-bullets">{value.map((entry, index) => <li key={index}><FieldValue value={entry} depth={depth + 1} /></li>)}</ul>
      : <ol className="field-entries">{value.map((entry, index) => <li key={index}><FieldValue value={entry} depth={depth + 1} /></li>)}</ol>;
  }
  if (isPlainObject(value)) {
    if (Object.keys(value).length === 0) return <span className="field-muted">Nothing recorded</span>;
    if (depth >= 3) return <pre className="field-long">{JSON.stringify(value, null, 2)}</pre>;
...
export function FieldList({ value, depth = 0 }: { value: unknown; depth?: number }) {
  if (value === null || value === undefined) return <p className="field-muted field-bare">Nothing recorded.</p>;
  if (isPlainObject(value)) {
    const entries = Object.entries(value);
    if (entries.length === 0) return <p className="field-muted field-bare">Nothing recorded.</p>;
    return <dl className={depth > 0 ? 'field-list is-nested' : 'field-list'}>
      {entries.map(([key, entry]) => <div className="field-row" key={key}>
        <dt>{humanizeKey(key)}</dt>
        <dd><FieldValue value={entry} depth={depth} /></dd>
      </div>)}
    </dl>;
  }
  return <div className="field-bare"><FieldValue value={value} depth={depth} /></div>;
}

/** Evidence travels in a few shapes across skills; read all of them, dump none. */
function readEvidence(entry: unknown): { label: string; detail: string; url: string | null } | null {
...
function EvidenceList({ entries }: { entries: unknown[] }) {
  return <div className="evidence-list">
    {entries.map((entry, index) => {
      const item = readEvidence(entry);
      if (!item) return <div className="evidence-item" key={index}><FieldList value={entry} /></div>;
      return <div className="evidence-item" key={index}>
        <strong>{item.label}</strong>
        {item.detail && <p>{item.detail}</p>}
        {item.url && <a href={item.url} target="_blank" rel="noreferrer">{item.url} <ExternalLink size={12} /></a>}
      </div>;
    })}
  </div>;
}

function readPolicyDecision(value: unknown): { effect: string; name: string; reason: string } | null {
...
function PolicyNote({ decision }: { decision: { effect: string; name: string; reason: string } }) {
  ...
  const sentence = decision.effect === 'require_approval'
    ? <>Needed your approval before anything could happen{named ? <> — that is your {named} limit</> : null}.</>
    : decision.effect === 'deny'
      ? <>Blocked{named ? <> by your {named} limit</> : null}. Nothing was sent.</>
      : <>Allowed{named ? <> by {named}</> : ' automatically'}.</>;
  return <div className="policy-note">
    <p><ShieldCheck size={16} /> <span>{sentence}</span></p>
...
function SignedNote({ value }: { value: string }) {
  ...
  return <div className="signed-note">
    <div>
      <code>{value.slice(0, 12)}…</code>
      <button className="ghost-button" onClick={() => void copy()}>{copied ? <><Check size={14} /> Copied</> : <><Copy size={14} /> Copy</>}</button>
    </div>
    <small>Your approval was pinned to this exact wording. If a single character changes afterwards, Trevra rejects it instead of sending it.</small>
  </div>;
}

export function RunSection({ title, children }: { title: string; children: React.ReactNode }) {
  return <section className="run-section"><h5>{title}</h5>{children}</section>;
}

export function FactGrid({ facts }: { facts: RunFact[] }) {
  return <div className="run-facts">
    {facts.map((fact) => <div className="run-fact" key={fact.label}><span>{fact.label}</span><strong>{fact.value}</strong></div>)}
  </div>;
}

function TimingRows({ rows }: { rows: RunFact[] }) {
  return <dl className="field-list">
    {rows.map((row) => <div className="field-row" key={row.label}><dt>{row.label}</dt><dd>{row.value}</dd></div>)}
  </dl>;
}
 ...
 */
function RunNodeCard({ node, index, onOpenStep }: { node: InspectorNode; index: number; onOpenStep: (skillRunId: string) => void }) {
  ...
  return <article className={`run-node${failed ? ' is-failed' : ''}`}>
    ...
    {open && <div className="run-node-body">
      {node.error && <div className="error-banner">
        <strong>What went wrong.</strong> {node.error}
        {node.attempt !== null && node.attempt > 1 ? ` Trevra tried this step ${node.attempt} times.` : ''}
      </div>}
      <RunSection title={node.inputLabel}><FieldList value={node.input} /></RunSection>
      <RunSection title={node.outputLabel}><FieldList value={node.output} /></RunSection>
      {node.evidence.length > 0 && <RunSection title="The proof behind it"><EvidenceList entries={node.evidence} /></RunSection>}
      {policy && <RunSection title="Why this was allowed"><PolicyNote decision={policy} /></RunSection>}
      {node.approvalPayloadHash && <RunSection title="What you signed"><SignedNote value={node.approvalPayloadHash} /></RunSection>}
      {node.facts.length > 0 && <RunSection title="Details"><FactGrid facts={node.facts} /></RunSection>}
      <RunSection title="Timing"><TimingRows rows={node.timing} /></RunSection>
      {node.skillRunId && <button className="secondary-button run-open" onClick={() => onOpenStep(node.skillRunId as string)}>Open this step on its own <ChevronRight size={15} /></button>}
      <details className="run-raw"><summary>Raw record</summary><pre>{JSON.stringify(node.raw, null, 2)}</pre></details>
    </div>}
  </article>;
}

export function RunInspector({ target, onClose }: { target: InspectorTarget; onClose: () => void }) {
    ...
    try {
      if (which.kind === 'auto') {
        ...
        if (!agentFirst) {
          ...
          if (job) { setRun(inspectorFromPlaybookRun(job)); setState('ready'); return; }
        }
        const agent = await getAgentRun(which.id).catch(() => null);
        if (agent) { setRun(inspectorFromAgentRun(agent)); setState('ready'); return; }
        if (agentFirst) {
          const job = await getPlaybookRun(which.id).catch(() => null);
          if (job) { setRun(inspectorFromPlaybookRun(job)); setState('ready'); return; }
        }
        const step = await getSkillRun(which.id).catch(() => null);
        if (step) { setRun(inspectorFromSkillRun(step)); setState('ready'); return; }
        setRun(null);
        setState('missing');
        return;
      }
      if (which.kind === 'playbook') {
        ...
        if (!detail) { setRun(null); setState('missing'); return; }
  ...
  useEffect(() => { void load(current); }, [current.kind, current.id]);
  ...
  return createPortal(<div className="drawer-backdrop" role="presentation" onClick={onClose}>
    <section ref={dialog} className="drawer drawer-wide" role="dialog" aria-modal="true" aria-labelledby="run-inspector-title" onClick={(event) => event.stopPropagation()}>
      <header>
        <div className="inspector-head">
          <span className="drawer-kicker"><Workflow size={14} /> {run?.kindLabel ?? 'Run'}</span>
          <h3 id="run-inspector-title">{run?.title ?? 'Opening…'}</h3>
          {run && <div className="inspector-meta">
        ...
        <div className="inspector-actions">
          {stack.length > 1 && <button className="icon-button" aria-label="Back" onClick={() => setStack((entries) => entries.slice(0, -1))}><ArrowLeft size={18} /></button>}
          <button className="icon-button" aria-label="Refresh" onClick={() => void load(current)}><RefreshCw size={17} /></button>
          <button className="icon-button" aria-label="Close" onClick={onClose}><X size={20} /></button>
        </div>
      </header>
      <div className="drawer-body">
        {state === 'loading' && <div className="empty-state"><LoaderCircle className="spin" size={28} /><h4>Opening this run…</h4><p>Fetching every step, what went in, and what came out.</p></div>}
        {state === 'missing' && <div className="empty-state"><ListTree size={28} /><h4>This run isn’t here</h4><p>It may have been cleared. Close this and pick another run from the list.</p></div>}
        {state === 'error' && <div className="empty-state"><CircleAlert size={28} /><h4>Could not open this run</h4><p>{message}</p><button className="secondary-button" onClick={() => void load(current)}><RefreshCw size={15} /> Try again</button></div>}
        {state === 'ready' && run && <>
          {run.error && <div className="error-banner"><strong>This run stopped.</strong> {run.error}</div>}
          ...
          {run.input !== null && run.input !== undefined && <RunSection title={run.inputLabel}><FieldList value={run.input} /></RunSection>}
          <RunSection title="Step by step">
            {run.nodes.length === 0
              ? <div className="empty-state"><ListTree size={26} /><h4>No steps yet</h4><p>{run.emptyNodes}</p></div>
              : <div className="run-timeline">
                  {run.nodes.map((node, index) => <RunNodeCard
                    key={node.key}
                    node={node}
                    index={index}
                    onOpenStep={(skillRunId) => setStack((entries) => [...entries, { kind: 'skill', id: skillRunId }])}
                  />)}
                </div>}
          </RunSection>
          {run.output !== null && run.output !== undefined && <RunSection title={run.outputLabel}><FieldList value={run.output} /></RunSection>}
          <RunSection title="Timing"><TimingRows rows={run.timing} /></RunSection>
        </>}
      </div>
    </section>
  </div>, document.body);
