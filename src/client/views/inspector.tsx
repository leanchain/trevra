...
export function buildSchemaValue(
  schema: SchemaNode,
  ...
  const walk = (node: SchemaNode, prefix: string[]): void => {
    ...
    if (current.type === 'boolean') {
      setPath(result, prefix, raw === 'true');
      return;
    }
...
function SchemaField({
  ...
  const control = current.enum ? (
    ...
    <input
      type={
        current.type === 'number' || current.type === 'integer'
          ? 'number'
          : current.format === 'email'
            ? 'email'
            : 'text'
      }
      value={value}
...
export function SchemaForm({
  ...
  if (!root.properties)
    return (
      <p className="schema-empty">{emptyCopy ?? 'This playbook takes no input. Just start it.'}</p>
    );
  return (
