...
function WindowChoice({ days, onChange, loading }: { days: number; onChange: (days: number) => void; loading: boolean }) {
  return <div className="li-filter-row" role="group" aria-label="How far back to count">
    ...
    {!total || !invites
      ...
      : !anything
        ? <div className="empty-state">
          ...
          <p>A slot enters this ledger when an approved plan is exported. Widen the window, or build a campaign and the funnel fills itself in.</p>
        ...
        : <>
          ...
          <div className="li-stat-row">
            ...
            <LiStat label="Invite acceptance" value={ratePercent(invites.invitesAccepted, invites.invitesSent)}
              detail={`${invites.invitesAccepted} accepted of ${invites.invitesSent} invites sent`} />
            <LiStat label="Replies from accepted" value={ratePercent(total.replied, total.accepted)}
          ...
          <p className="panel-note">
            <b>Invite acceptance</b> is accepted out of invites sent — the same figure the Campaigns screen shows. The
            acceptance meter on <a className="li-link" href="/outreach/safety">account safety</a> is a different number
            on purpose: it counts only invites that were answered, because that is what Trevra throttles on. A rate is
            shown as “{NOT_ENOUGH_DATA}” until there are {RATE_MIN_SAMPLE} in the denominator.
          </p>
        </>}
  </section>;
}
...
export function LinkedInCampaignBreakdown() {
  ...
  return <section className="page-panel">
    <div className="section-heading">
      <div>
        ...
        <p>
          ...
          Acceptance is accepted out of invites sent, so a campaign whose invites are still unanswered reads low rather
          than blank.
        </p>
      </div>
    </div>
    ...
    {!analytics || analytics.byCampaign.length === 0
      ? <p className="empty-copy">{loading ? 'Reading the outreach ledger…' : error ? 'No count to show.' : 'No campaign has filed an action in this window.'}</p>
      : <div className="li-table-scroll">
        <table className="li-table">
          ...
          <tbody>{analytics.byCampaign.map((row) => <tr key={row.campaignId}>
            ...
            <td className="li-num" title={`${row.invitesAccepted} accepted of ${row.invitesSent} invites sent`}>
