...
function taskLabel(task: string): string {
  return (
    MAINTENANCE_TASK_LABELS[task as keyof typeof MAINTENANCE_TASK_LABELS] ??
...
function runSummary(run: LinkedInBackgroundRunHistoryItem): string {
  if (run.kind === 'actions') {
    ...
    if (run.executedCount > 0)
      return `${run.executedCount} LinkedIn action${run.executedCount === 1 ? '' : 's'} completed.`;
...
function RunRow({ run }: { run: LinkedInBackgroundRunHistoryItem }) {
  ...
  const duration =
    finished && Number.isFinite(started.getTime()) && Number.isFinite(finished.getTime())
      ? Math.max(0, Math.round((finished.getTime() - started.getTime()) / 60_000))
      : null;
  return (
    <div className="li-activity-run">
...
export function OutreachActivity() {
  ...
  const load = useCallback(
    async (quiet = false) => {
      ...
      try {
        const next = await getLinkedInActivity(100, activeSeatKey);
        setData(next);
        setError('');
      } catch (cause) {
  ...
  if (loading && !data)
    return (
      <div className="center-state">
        ...
        <p>Loading LinkedIn activity…</p>
      </div>
    );

  const next = data?.nextRun ?? null;
  const nextWindow =
    next && next.source !== 'catchup'
      ? formatVisitWindow(next.startAt, next.endAt, next.timezone)
      : null;
  const blocker = next ? queueWaitCopy(next.waitingFor) : null;
  const readyNow = Boolean(next?.source === 'catchup' && !blocker);

  return (
    <div className="page-stack li-activity-page">
      <section className="page-panel li-activity-next">
        <div className="section-heading">
          <div>
            <h2>
              <Clock3 size={18} /> Next LinkedIn background run
            </h2>
            <p>
              The next real browser sitting Trevra expects to use for the selected LinkedIn account.
            </p>
          </div>
        ...
        {next ? (
          <div className="li-activity-next-value">
            <strong>
              {blocker ?? (readyNow ? 'Ready now · catch-up' : (nextWindow ?? 'Scheduled'))}
            </strong>
            <span>
              {next.source === 'actions'
                ? 'Scheduled outreach'
            ...
            {blocker && next.source === 'catchup' && (
              <small>The catch-up will run as soon as this prerequisite is back.</small>
            )}
      ...
      <section className="page-panel">
        <div className="section-heading">
          <div>
            ...
            <p>
              Actual LinkedIn browser sittings. No page content, cookies, passwords or message
              bodies are stored here.
            </p>
          </div>
        </div>
        {data?.runs.length ? (
          <div className="li-activity-runs">
            {data.runs.map((run) => (
              <RunRow run={run} key={`${run.kind}:${run.id}`} />
            ))}
          </div>
        ) : (
          <div className="empty-state compact">
