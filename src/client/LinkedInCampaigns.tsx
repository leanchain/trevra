...
const STEP_KINDS: readonly StepKindMeta[] = [
   ...
   */
  {
    ...
    carriesCopy: false,
    noCopyNote:
      'A follow carries no message. It puts you in their notifications without asking for anything.'
  },
  {
    kind: 'like',
    label: 'Like',
    ...
    carriesCopy: false,
    noCopyNote:
      'A like carries no message. It lands on their most recent post, and LinkedIn tells nobody whether it was noticed.'
  },
  {
    kind: 'endorse',
    label: 'Endorse',
    ...
    carriesCopy: false,
    noCopyNote:
      'An endorsement carries no message. It endorses a skill already on their profile — nothing here writes one.'
  }
];
...
function eligibleAnchors(
  ...
  if (on === 'accepted' || on === 'not_accepted')
    return earlier.filter((step) => step.kind === 'invite');
...
function branchProblem(steps: readonly EditableSequenceStep[], index: number): string | null {
  ...
  if (!ofStepId)
    return 'This branch names no step to wait on, so nothing would decide whether it runs.';
  ...
  if (!RESULT_BEARING_KINDS.includes(anchor.kind)) {
    return (
      `A ${stepKindMeta(anchor.kind).label.toLowerCase()} is never accepted or replied to, so this branch could never be ` +
...
function normalizeSteps(value: unknown): EditableSequenceStep[] {
  ...
  list.forEach((entry, index) => {
    ...
    const kind = STEP_KINDS.some((meta) => meta.kind === step.kind)
      ? (step.kind as SequenceStepKind)
      : 'dm';
    const day = Number.isFinite(Number(step.day))
      ? Math.max(0, Math.trunc(Number(step.day)))
      : index * 2;
    normalized.push({
      ...
      template:
        stepKindMeta(kind).carriesCopy && typeof step.template === 'string' ? step.template : '',
...
function normalizeCondition(
  value: unknown,
  earlier: readonly EditableSequenceStep[]
): StepCondition | null {
  if (!value || typeof value !== 'object') return null;
  const candidate = value as Partial<StepCondition>;
  const on =
    typeof candidate.on === 'string' &&
    (LINKEDIN_BRANCH_ON as readonly string[]).includes(candidate.on)
...
function applyDraftBrief(
  ...
  return {
    ...
    proof:
      isDegraded(degraded, 'offer', 'proof') || !Array.isArray(offer.proof)
        ? ''
        : offer.proof
            .filter(
...
function applySuggestedBrief(
  brief: CampaignBrief,
  suggested: LinkedInCampaignDraft['suggested']
): CampaignBrief {
...
function storedCampaignInput(input: unknown): StoredCampaignInput | null {
  ...
  return {
    ...
    brief: {
      ...
      horizonDays: Number.isFinite(horizon)
        ? Math.max(1, Math.trunc(horizon))
        : EMPTY_BRIEF.horizonDays,
...
function BindingCeiling({
  ...
  const ceiling: LinkedInCeiling | undefined = limits?.limits.find(
    (limit) => limit.kind === kind && limit.window === 'day'
  );
  if (!ceiling) {
    ...
    if (error) {
      return (
        <small className="li-hint">
          {error} Until that read succeeds this line cannot say how many are left today, and nothing
          here assumes it is zero.
        </small>
      );
    }
    if (limits && !limits.seat.configured) {
      return (
        <small className="li-hint">
          No LinkedIn account is connected yet, so no daily limit applies. Connect one and this line
          says how many are left today, and why.
        </small>
      );
    }
    return (
      <small className="li-hint">
        No daily limit came back for {KIND_LABELS[kind].toLowerCase()} on this account, so this line
        cannot say how many are left today.
      </small>
    );
  }
  ...
  const overruled =
    ceiling.ceilingSource === 'band' &&
    ceiling.operatorLimit !== null &&
    ceiling.operatorLimit !== undefined &&
    ceiling.operatorLimit > ceiling.bandCeiling;

  return (
    <small className="li-hint">
      {ceiling.remaining} of {ceiling.ceiling} {KIND_LABELS[kind].toLowerCase()} left today —{' '}
      {limitReason(ceiling.boundBy)}.
      {overruled && (
        <>
          {' '}
          You set {ceiling.operatorLimit} a day for this account and{' '}
          <b>{ceiling.bandCeiling} is what goes out</b> — Trevra’s researched ceiling is the lower
          of the two, so it binds. Whose number binds is a per-account switch on Safety.
        </>
      )}{' '}
      Spreading over more days never raises that number, so too few days simply means fewer people
      get reached.{' '}
      <ConfidenceTag
        confidence={ceiling.confidence}
        source={sourceNote(ceiling.confidence)}
        compact
      />
    </small>
  );
...
export function OutreachCampaigns({
  ...
  const loadList = async () => {
    try {
      setCampaigns(await getLinkedInCampaigns(seatKey || undefined));
    } catch (err) {
      setError(errorMessage(err, 'Unable to load LinkedIn campaigns'));
    }
  };
  ...
  const openCampaign = async (id: string) => {
    ...
    try {
      ...
      setError(errorMessage(err, 'Unable to load that campaign'));
    } finally {
      setBusy(null);
    }
  };
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        setSequenceConfig(await getLinkedInSequenceConfig());
      } catch {
        /* the branch vocabulary has a local fallback */
      }
    })();
  }, []);
  ...
  useEffect(() => {
    ...
    setToast(
      `${staged.length} profile URL(s) from your lead search are in the list below. ` +
  ...
  const addStep = (kind: SequenceStepKind) =>
    setSteps((current) => [
      ...current,
      {
  ...
  const insertMerge = (id: string, field: string) => {
    ...
    setSteps((current) =>
      current.map((step) => {
        ...
        return {
          ...step,
          template: `${step.template.slice(0, from)}${token}${step.template.slice(to)}`
        };
      })
    );
   ...
   */
  const orderedSteps = (): EditableSequenceStep[] =>
    [...steps]
  ...
  const rejectedFields = [
    ...new Set(steps.flatMap((step) => unknownMergeFields(step.template, mergeFields)))
  ];
  ...
  const branchIssues = steps
    ...
    .filter(
      (entry): entry is { step: EditableSequenceStep; problem: string } => entry.problem !== null
    );
  ...
  const draftSequence = async () => {
    ...
    try {
      ...
      setBrief((current) =>
        applySuggestedBrief(applyDraftBrief(current, result?.brief, missing), suggested)
      );
  ...
  const toggleTemplates = async () => {
    ...
    if (sequenceConfig) {
      setTemplates(sequenceConfig.templates);
      return;
    }
    setBusy('templates');
    setError('');
    try {
      ...
      setError(errorMessage(err, 'Unable to load sequence templates'));
    } finally {
      setBusy(null);
    }
  };
  ...
  const useTemplate = (template: LinkedInSequenceTemplate) => {
    ...
    setDraftTemplateId(null);
    setToast(
      `Loaded “${template?.name ?? 'template'}”. Every line is yours to edit — nothing is saved yet.`
    );
  };
  ...
  const toggleLeadSources = async () => {
    ...
    try {
      ...
      setError(errorMessage(err, 'Unable to read the lead sources'));
    } finally {
      setBusy(null);
    }
  };
  ...
  const pullLeadSource = async (source: LinkedInLeadSource) => {
    ...
    try {
      ...
      if (pulled.length === 0) {
        setToast(
          'That search found nobody, so nothing was added. Trevra will not invent people to fill the gap.'
        );
        return;
      }
      ...
      setToast(
        `${added} profile URL(s) added (${pulled.length - added} were already there). ` +
          'Nothing is saved until you create the campaign.'
      );
      setLeadSources(null);
    } catch (err) {
      setError(errorMessage(err, 'Unable to read the people that source found'));
    } finally {
      setBusy(null);
    }
  };
  ...
  const uploadTargets = async (file: File) => {
    ...
    try {
      ...
      setToast(
        `${result.parsed} row(s) read, ${result.targets.length} usable. ` +
  ...
  const create = async () => {
    ...
    if (overStepCap) {
      setError(
        `A sequence may have at most ${maxSteps} steps and this one has ${steps.length}. ` +
    ...
    const missing = DRAFTABLE_BRIEF.filter(([field]) => !String(brief[field]).trim()).map(
      ([, label]) => label
    );
    if (assembled.length === 0 && missing.length > 0) {
      setError(
        'Add a step below, start from a template, or draft one with AI — a campaign is its sequence. ' +
          ...
          assembled.length > 0
            ...
            : {
                ...
                icp: {
                  ...
                  segment: brief.segment.trim(),
                  pain: brief.pain.trim()
                },
                offer: {
  ...
  const saveSequence = async () => {
    if (!boundId) return;
    if (steps.length === 0) {
      setError('A sequence needs at least one step.');
      return;
    }
    if (rejectedFields.length > 0) {
      setError(
  ...
  const queue = async (campaignId: string): Promise<boolean> => {
    ...
    try {
      ...
      setToast(
        `${result.recorded?.written ?? 0} action(s) queued for the worker on your machine, ` +
  ...
  const stop = async (campaignId: string): Promise<boolean> => {
    ...
    try {
      ...
      setToast(
        `Campaign stopped. ${result.releasedActions} scheduled action(s) will not go out. This stops one campaign, not the account — Pause everything, in the bar at the top of every screen, does that.`
      );
      await reloadOutreach();
      return true;
    } catch (err) {
    ...
    <div className="page-stack">
      <section className="li-dryrun">
        ...
        <div>
          ...
          <p>
            This is the advanced path, and nothing on it goes out on its own. Use it when you want
            to run the messages in another tool, or when Trevra cannot open a browser for you. For
            everyday outreach — a lead list, a workflow, and a Start button that works through it
            for you — use{' '}
            <a className="li-link" href="/outreach/manager">
              Campaigns
            </a>
            .
          </p>
      ...
      <section className="page-panel">
        ...
        {campaigns.length === 0 ? (
          <p className="empty-copy">
            Nothing built here yet. Name one below, then draft a sequence or assemble it step by
            step.
          </p>
        ) : (
          <div className="li-campaign-list">
            {campaigns.map((campaign) => (
              ...
              >
                ...
                <span className="li-campaign-date">
        ...
        <section className="page-panel">
          <div className="section-heading">
            ...
            <div className="li-detail-actions">
              ...
              >
                {LINKEDIN_EXPORT_FORMATS.map((option) => (
              ...
              <button
                ...
                onClick={() => {
                  setError('');
                  setConfirmQueue({ id: detail.campaign.id, name: detail.campaign.name });
                }}
              >
                {busy === 'queue' ? (
                  <LoaderCircle className="spin" size={14} />
                ...
                Send with my own worker
              </button>
              <button
                className="ghost-button"
                disabled={busy === 'stop' || detail.campaign.status === 'stopped'}
                onClick={() => {
                  setError('');
                  setConfirmStop({ id: detail.campaign.id, name: detail.campaign.name });
                }}
              >
          ...
          {!approved && (
            <p className="li-hint">
              Exporting and queueing both hand over the exact bytes a human approved, and this
              campaign has none yet — so both stay disabled until the plan below is approved.
              Neither could do anything before then but refuse.
            </p>
          )}
          ...
          {sequence ? (
            <>
              ...
              <p className="li-hint">
                {sequence.steps.length} step(s), loaded into the builder below. Edit them there and
                save.
              </p>
            </>
          ) : (
                    ...
                    {detail.exports.map((record) => (
                      <tr key={record.id}>
                        ...
                        <td>
                          <a
                            className="li-link"
                            href={linkedInExportDownloadPath(record.campaignId, record.id)}
                          >
                            Download
                          </a>
                        </td>
          ...
          <h4 className="li-subhead" aria-level={3}>
            Scheduled actions ({detail.actions.length})
          </h4>
          {detail.actions.length === 0 ? (
            <p className="empty-copy">
              Nothing is scheduled for this campaign yet. Actions are scheduled when an approved
              plan is exported.
            </p>
          ) : (
            <div className="li-table-scroll">
              <table className="li-table">
                ...
                <tbody>
                  {detail.actions.slice(0, 50).map((action) => (
                    <tr key={action.id}>
                      <td className="li-target">{action.targetRef ?? '—'}</td>
                      <td>{ACTION_KIND_LABELS_ONE[action.kind]}</td>
                      <td>
                        <span className={`li-chip li-status-${action.status}`}>
                          {actionStatusLabel(action.status)}
                        </span>
                      </td>
                      <td>
                        {action.plannedFor ? new Date(action.plannedFor).toLocaleString() : '—'}
                      </td>
                    </tr>
                  ))}
                </tbody>
      ...
      <section className="page-panel">
          ...
          <div>
            ...
            <p>
              {boundId
                ...
                : 'A name, your domain, and who to reach. The copy is written below — the brief is optional and only steers the AI draft.'}
            </p>
          </div>
        </div>
        ...
        <div className="li-form-grid">
          <label>
            ...
            <input
              ...
              placeholder="Q3 RevOps founders"
            />
          </label>
          <label>
            Your domain
        ...
        <div className="li-form-grid">
          <label className="li-span-2">
            People to reach — one handle or profile URL per line
            <textarea
              rows={6}
              value={targets}
          ...
          <div className="li-form-side">
            ...
            <p className="panel-note">
              Importing saves nothing. It reads the file, drops anyone on your Never contact list,
              tells you who this account has already contacted, and leaves the usable list here for
              you to read before any of it becomes a plan.
              {contacts.length > 0 && (
                <> {contacts.length} contact record(s) held for the export’s merge fields.</>
              )}
            </p>
          </div>
        </div>
        ...
        {leadSources !== null && (
          <div className="li-lead-picker">
            ...
            {leadSources.length === 0 ? (
              <p className="empty-copy">
                No search has been run yet. Start one on Find leads, then pull the results in here.
              </p>
            ) : (
                  ...
                  >
                    <span className="li-source-top">
                      <span className="li-chip">
                        {LEAD_SOURCE_KINDS[source.kind] ?? source.kind}
                      </span>
                      <span className={`li-chip li-lead-${source.status}`}>
                        {LEAD_SOURCE_STATUS[source.status] ?? source.status}
                      </span>
            ...
            <p className="panel-note">
              Pulling adds the profile URLs this search found to the list above and creates nothing.
              Your Never contact list is checked where the plan is built, so anybody who asked to be
              left alone is dropped before a message exists.
            </p>
          </div>
        )}
        ...
        <div className="li-create-actions">
          ...
          <small className="li-hint">
            Both fill the builder below, and both are a starting point — every step stays editable.{' '}
            <b>Draft with AI</b> reads your domain and nothing else: the tone and the two draft
            options in the brief below are read where Trevra writes the sequence from the brief
            instead, when you create the campaign.
          </small>
        </div>
        ...
        {templates !== null &&
          (templates.length === 0 ? (
            <p className="empty-copy">
              No sequence templates are published yet. Add steps by hand below, or draft one.
            </p>
          ) : (
            <div className="li-template-list">
              {templates.map((template) => (
                <article className="li-template-card" key={template?.id ?? template?.name}>
                  <div>
                    <strong>{template?.name ?? 'Untitled template'}</strong>
                    <p>{template?.description ?? ''}</p>
                    <div className="li-template-steps">
                      {(Array.isArray(template?.steps) ? template.steps : []).map((step, index) => (
                        <span
                          className="li-chip"
                          key={`${template?.id ?? 'template'}-${step?.id ?? index}`}
                        >
                          Day {Number.isFinite(Number(step?.day)) ? Number(step.day) : 0} ·{' '}
                          {stepKindMeta(step?.kind).label}
                        </span>
                      ))}
                    </div>
                  </div>
                  <button className="secondary-button" onClick={() => useTemplate(template)}>
                    Use this
                  </button>
                </article>
              ))}
        ...
        <details
          className="li-brief-fold"
          open={briefOpen}
          onToggle={(event) => setBriefOpen(event.currentTarget.open)}
        >
          <summary>
            <Sparkles size={13} /> Fine-tune the brief
          </summary>
          <p className="li-hint">
            {boundId
              ...
              : 'The action type, spread and format below travel with the campaign when you create it. The ICP, offer and ' +
                ...
                'Trevra writes one from them instead. Build the sequence below and none of that is needed.'}
          </p>
          ...
          {degraded.length > 0 && (
            <div className="li-degraded">
              ...
              <p>
                They are not stated on your site.{' '}
            ...
            <div className="li-suggested">
              ...
              <div>
                ...
                <p>
                  ...
                  states what a company does and never who it sells to. READ THEM BEFORE THEY GO
                  ANYWHERE — they are the only fields on this screen with nothing behind them, and
                  they are yours to rewrite or clear. Nothing about your proof was suggested: that
          ...
          <p className="li-hint">
            {boundId
              ...
              : 'These three travel with every campaign, whichever way its copy arrives. Each already has a working default ' +
                '— change one when you have a reason to.'}
          </p>
          ...
          <div className="li-form-grid">
            <label>
              ...
              <Define term="Action type">
                the action this campaign spends its daily allowance on. Each one has its own daily
                limit, and the Account screen says what is left of it.
              </Define>
            </label>
            <label>
              Spread over (days)
              <input
                ...
                min={1}
                max={90}
                value={brief.horizonDays}
                disabled={Boolean(boundId)}
                onChange={(event) =>
                  setBrief({ ...brief, horizonDays: Number(event.target.value) || 1 })
                }
              />
              <BindingCeiling
                limits={limits}
            ...
            <label>
              ...
              <select
                ...
                disabled={Boolean(boundId)}
                onChange={(event) =>
                  setBrief({ ...brief, format: event.target.value as ExportFormat })
                }
              >
                {LINKEDIN_EXPORT_FORMATS.map((option) => (
                  <option key={option} value={option}>
              ...
              <Define term="Export format">
                which tool’s CSV an approved plan is written for. Trevra sends nothing on this
                screen; you run the file, or you queue it for your own worker.
              </Define>
            </label>
          </div>
          ...
          <details className="li-manual-fields">
            <summary>
              <Sparkles size={13} /> Have Trevra draft the sequence from a brief instead
            </summary>
            <p className="li-hint">
              Read only when there is no step below to save. The six fields marked <b>*</b> are
              required on this path and required together — Trevra writes the sequence from them,
              and one left empty is one the draft will invent. Build the sequence yourself below and
            ...
            <div className="li-form-grid">
              ...
              <label className="li-span-2">
                Why it works — the mechanism, not the outcome *
                <input
                  aria-required="true"
              ...
              <label className="li-span-2">
                Proof, one <code>label: value</code> per line
                <textarea
                  rows={3}
                  value={brief.proof}
                  disabled={Boolean(boundId)}
            ...
            <h4 className="li-subhead" aria-level={3}>
              How that draft should read
            </h4>
            <p className="li-hint">
              Read only on this same path. A sequence you built, loaded from a template or drafted
              from your domain already carries its own copy, and none of these three change a word
              of it.
            </p>
            ...
            <div className="li-form-grid">
              <label>
                ...
                <select
                  ...
                  disabled={Boolean(boundId)}
                  onChange={(event) =>
                    setBrief({ ...brief, tone: event.target.value as SequenceTone })
                  }
                >
                  {TONES.map((tone) => (
                ...
                <Define term="Tone">
                  how the drafted copy addresses the target. It is read only where Trevra writes the
                  sequence from the brief above; it changes nothing about copy you wrote, and
                  nothing about what is sent or when.
                </Define>
              </label>
            </div>
            ...
            <label className="li-inline-check">
              <input
                ...
                disabled={Boolean(boundId)}
                onChange={(event) =>
                  setBrief({ ...brief, inviteNote: event.target.checked ? 'none' : 'drafted' })
                }
              />
              <span>Have the draft write the connection request with no note</span>
            </label>
            <small className="li-hint">
              On by default. A note written before anyone has accepted anything is the most-read
              line in the sequence and the least earned; a templated one reads as templated.
              Unticking this asks the draft for a note you can then edit. To send a noteless invite
              in a sequence you built yourself, tick the same box on the invite card below — an
              invite with empty copy IS a noteless invite.
            </small>
      ...
      <section className="page-panel">
          ...
          <div>
            ...
            <p>
              {steps.length === 0
                ...
                : `${steps.length} step(s) across ${lastDay + 1} day(s), in day order. Day 0 is the day the campaign starts.`}
            </p>
          </div>
        </div>
        ...
        {draftTemplateId && (
          <p className="li-hint">
            These steps came from the{' '}
            <b>
              {sequenceConfig?.templates.find((template) => template.id === draftTemplateId)
                ?.name ?? draftTemplateId}
            </b>{' '}
            template rather than being written for your brief — the draft had no complete ICP and
        ...
        {steps.length === 0 ? (
          <p className="empty-copy">
            Nothing here yet. A sequence is a list of actions with the gap you choose between them.
          </p>
        ) : (
          <div className="li-sequence">
            {steps.map((step, index) => (
              <SequenceStepCard
                ...
                onSort={sortSteps}
                bindCopy={(element) => {
                  copyRefs.current[step.id] = element;
                }}
          ...
          <div className="li-add-step-groups">
            ...
            <fieldset className="li-add-step-group" disabled={atStepCap}>
              ...
              <div className="li-add-step-buttons">
                {STEP_KINDS.filter(({ carriesCopy }) => carriesCopy).map(
                  ({ kind, label, Icon }) => (
                    <button className="li-mini-button" key={kind} onClick={() => addStep(kind)}>
                      <Icon size={12} /> {label}
                    </button>
                  )
                )}
        ...
        <p className="li-hint">
          <b>Follow</b>, <b>like</b> and <b>endorse</b> carry no copy, like a profile view. They
          still count against their own daily limits and still go through every safety check —
          liking two hundred posts in an hour is a ban signal however harmless one like is.{' '}
          <ConfidenceTag confidence="REPORTED" source={sourceNote('REPORTED')} compact />
        </p>

        <p className="li-hint li-merge-legend">
          Merge fields are filled in by whichever tool runs the file, never here. These are the
          whole set —{' '}
          {mergeFields.map((field) => (
            <code key={field}>{`{{${field}}}`}</code>
          ))}{' '}
          — and Trevra rejects any other. LinkedIn cuts a connection-request note off at{' '}
          {inviteNoteMax} characters.{' '}
        ...
        <div className="panel-footer">
          <span>
            {boundId
              ...
              : 'Creating saves these steps as the campaign’s sequence, checks them against every safety limit, and then stops for your approval.'}
          </span>
          {boundId ? (
            <button
              className="primary-button"
            ...
            <button
              ...
              onClick={() => void create()}
            >
              {busy === 'create' ? <LoaderCircle className="spin" size={15} /> : <Play size={15} />}{' '}
              Create campaign
            </button>
          )}
        </div>
      ...
      {confirmStop && (
        <ConfirmDrawer
          ...
          body={
            <>
              <p>
                Everything this campaign still has scheduled is cancelled — marked skipped, never
                sent — and it stops scheduling anything new. This stops one campaign, not the
                account.
              </p>
          ...
          onConfirm={() =>
            void stop(confirmStop.id).then((ok) => {
              if (ok) setConfirmStop(null);
            })
          }
        />
      )}
      ...
      {confirmQueue && (
        <ConfirmDrawer
          ...
          body={
            <>
              <p>
                This files the approved actions for the worker running on your machine.{' '}
                <b>Nothing is sent by pressing it.</b> The worker takes them one at a time, re-runs
                the whole safety gate against each immediately before acting, and works only inside
                this account’s working hours.
              </p>
              <p>
                What goes out is exactly what you approved — that wording, those people, that
                schedule. Queueing the same campaign twice queues nothing twice: an action this seat
                already has for a person is recognised and skipped.
              </p>
              <p>
                Self-hosted only. On a hosted Trevra this is refused, and the export beside it is
          ...
          onConfirm={() =>
            void queue(confirmQueue.id).then((ok) => {
              if (ok) setConfirmQueue(null);
            })
          }
        />
      )}
      ...
      {confirmRemove && (
        <ConfirmDrawer
            ...
            <>
              ...
              <p>
                {boundId
                  ...
                  : 'This changes the builder only — no campaign exists yet, so nothing on the server changes either way.'}
              </p>
            </>
          }
          ...
          onConfirm={() => {
            ...
            setToast(
              `Step deleted from the builder. ${boundId ? 'The saved campaign is unchanged until you save the sequence.' : 'Nothing was saved.'}`
            );
          }}
...
function SequenceStepCard({
  ...
  const changeBranch = (on: BranchOn) => {
    ...
    const keep =
      condition && candidates.some((candidate) => candidate.id === condition.ofStepId)
        ? condition.ofStepId
  ...
  return (
    <article className={`li-step li-step-card${condition ? ' li-step-branched' : ''}`}>
      <header>
        <span className={`li-chip li-kind-chip li-kind-${step.kind}`}>
          <Icon size={12} /> {meta.label}
        </span>
        {condition && (
          <span className="li-chip li-branch-chip">
        ...
        <label className="li-day-control">
          ...
          <input
            ...
            onChange={(event) =>
              onChange({ day: Math.max(0, Math.trunc(Number(event.target.value) || 0)) })
            }
            onBlur={onSort}
          />
        </label>
              ...
              {branchOn.map((value) => {
                ...
                return (
                  <option key={value} value={value} disabled={unavailable}>
                    {BRANCH_ON_LABELS[value]}
                    {unavailable ? ' — no earlier step can answer this' : ''}
                  </option>
                );
              })}
            </select>
          ...
          {condition && (
            <label className="li-branch-control">
              ...
              <select
                ...
                onChange={(event) =>
                  onChange({ condition: { on: condition.on, ofStepId: event.target.value } })
                }
              >
                {anchors.map((anchor) => (
              ...
              did not write. */}
                {!anchors.some((anchor) => anchor.id === condition.ofStepId) && (
                  <option value={condition.ofStepId}>
      ...
      {carriesCopy ? (
        <>
          <div className="li-merge-row">
            ...
            {step.kind === 'invite' && (
              <span className={`li-count ${overLimit ? 'is-over' : ''}`}>
          ...
          {overLimit && (
            <small className="li-merge-warn">
              LinkedIn truncates the note at {inviteNoteMax} characters. A cut mid-sentence reads
              worse than one you shortened yourself.
            </small>
          )}
          {rejected.length > 0 && (
            <small className="li-merge-warn">
        ...
        <p className="li-hint">
          {bareInvite
            ...
            : (meta.noCopyNote ?? 'This step carries no message.')}
        </p>
      )}
    </article>
  );
...
function ApprovalBlock({
  ...
  return (
    <div className={`li-approval ${waiting ? 'is-waiting' : ''}`}>
      <div>
        <strong>
          {waiting
            ? 'This plan is waiting for you'
            : `Approval step: ${step.status.replaceAll('_', ' ')}`}
        </strong>
        <p>
          Approving covers exactly what is below — this wording, these people, this schedule, this
          file format. Change any of it afterwards and this approval stops counting: Trevra will ask
          you again rather than reuse it.
        </p>
      </div>
      {/* This button BINDS a hash. It used to be the same 34px green rectangle as
        ...
        on. */}
      {waiting && (
        <div className="li-approval-actions">
          <button
          ...
          >
            {busy ? <LoaderCircle className="spin" size={14} /> : <ShieldCheck size={14} />} Approve
            exactly this
          </button>
          <button
            className="ghost-button"
...
export function OutreachPlan({ setToast }: { setToast: (message: string) => void }) {
  ...
  const run = async () => {
    const list = splitTargets(targets);
    if (list.length === 0) {
      setError('Add at least one person to plan for.');
      return;
    }
    setBusy(true);
    setError('');
    try {
      const response = await planLinkedIn({
        kind,
        targets: list,
        horizonDays,
        ...(seatKey ? { seatKey } : {})
      ...
      setResult(response);
      setToast(
        `Worked out ${response.plan.slots.length} action(s) across ${horizonDays} days. Nothing was saved.`
      );
    } catch (err) {
      setResult(null);
    ...
    <div className="page-stack">
      <section className="li-dryrun">
        ...
        <div>
          ...
          <p>
            It reads what this account has actually done, works out a schedule against every limit
            at once, and then throws the answer away. Nothing is saved, nothing is scheduled, and
            nobody is contacted.
          </p>
        </div>
      </section>
      ...
      <section className="page-panel">
        ...
        <label className="li-block-label">
          People — one handle or profile URL per line
      ...
      {result && (
        <>
          <section className="page-panel">
            ...
            <ol className="li-reasons">
              {result.plan.reasons.map((reason) => (
                <li key={reason}>{reason}</li>
              ))}
            </ol>
            <div className="li-chip-row">
              {result.plan.ceilingsApplied.map((ceiling) => (
                <span className="li-chip" key={ceiling}>
            ...
            {result.excluded.length > 0 && (
              <p className="panel-note">
                {result.excluded.length} person(s) were dropped before planning because they are on
                your Never contact list:{' '}
                {result.excluded.map((entry) => entry.targetRef).join(', ')}. That list is checked
                before a plan exists, so nobody on it ever reaches a queue.
              </p>
            )}
          </section>
          ...
          <section className="page-panel">
            <div className="section-heading">
              <div>
                <h3 aria-level={2}>
                  {result.plan.slots.length} action(s) across {days.length} day(s)
                </h3>
                ...
                <p>
                  {seatZone ? (
                    <>
                      Days and times are this account’s own, in <b>{seatZone}</b> — the clock its
                      working hours and its daily ceiling are enforced against. Every one of them
                      was placed inside those working hours.
                    </>
                  ) : (
                    <>
                      This account’s timezone could not be read, so the days and times below are
                      your browser’s. The plan itself was still placed inside the account’s working
                      hours, in the account’s own clock.
                    </>
                  )}
                </p>
              </div>
            ...
            {days.length === 0 ? (
              <p className="empty-copy">
                The limits that apply right now leave no room to schedule anything. The Account
                screen says which one is holding it up.
              </p>
            ) : (
              <div className="li-calendar">
                {days.map(([day, slots]) => (
                  <div className="li-cal-day" key={day}>
                    <header>
                      <strong>{calendarDayLabel(day)}</strong>
                      <span>{slots.length}</span>
                    </header>
                    <i
                      className="li-cal-load"
                      style={{ width: `${(slots.length / busiest) * 100}%` }}
                    />
                    <ul>
                      {slots.map((slot) => (
                        <li key={`${slot.plannedFor}-${slot.targetRef}`}>
                    ...
                    omitting the key are the same to Intl only by convention. */}
                          <time>
                            {new Date(slot.plannedFor).toLocaleTimeString(
                              [],
                              seatZone
