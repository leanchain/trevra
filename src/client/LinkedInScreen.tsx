...
import { ActionRow, FormGrid } from './ui/forms';
...
export function LinkedInExclusions({ setToast }: { setToast: (message: string) => void }) {
  ...
  return (
    <Panel
      title="Never contact"
      description="People Trevra must never include in LinkedIn outreach."
      actions={
        <InlineActions>
          {exclusions.length > 0 && (
    ...
    >
      ...
      {showAdd && (
        <>
          <FormGrid>
            <Field label="Handles or profile URLs, one per line">
              <Textarea
                rows={4}
                value={targets}
          ...
          <ActionRow note="There is no Remove button here — taking somebody off this list is a database change, on purpose.">
            <Button
              variant="ghost"
              disabled={busy}
              onClick={() => {
            ...
            <Button variant="primary" disabled={busy} onClick={() => void add()}>
              {busy ? <LoaderCircle className="spin" size={15} /> : <Ban size={15} />} Add to the
              list
            </Button>
          </ActionRow>
        </>
      ...
      <div className="never-contact-list">
        {!loaded ? (
          ...
          <EmptyState description="No one is on the never-contact list yet." />
        ) : exclusions.length > 0 ? (
