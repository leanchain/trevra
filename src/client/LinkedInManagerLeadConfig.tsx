...
import {
  ArrowRight, Check, CircleAlert, FileUp, LoaderCircle, Pencil, RefreshCw, Trash2, Upload, Users, X
} from 'lucide-react';
import {
...
import type { LeadListSourceKind, LinkedInLeadContact, LinkedInLeadList } from '../server/linkedin/lead-lists';
...
async function readCsvHead(file: File): Promise<{ headers: string[]; rows: Array<Record<string, string>> }> {
...
function fileProblem(file: File): string {
  if (!/\.csv$/i.test(file.name)) return `“${file.name}” is not a .csv file. Export your list as CSV and drop it here again.`;
  if (file.size === 0) return `“${file.name}” is empty.`;
  if (file.size > MAX_CSV_BYTES) return `“${file.name}” is ${formatSize(file.size)}, and 2 MB is the most one upload takes. Split it and import the parts one after another.`;
  return '';
}
...
export function LinkedInManagerLeadConfig({ onChanged, setToast }: { onChanged: () => Promise<void>; setToast: (message: string) => void }) {
  ...
  const [sample, setSample] = useState<{ headers: string[]; rows: Array<Record<string, string>> } | null>(null);
  ...
  const [report, setReport] = useState<{ listName: string; listId: string; counts: ImportReport } | null>(null);
  ...
  const loadLists = useCallback(async (): Promise<LinkedInLeadList[]> => {
    ...
    try {
      const next = await getLinkedInManagerLeadLists();
      setLists(next);
      return next;
    } catch (err) {
  ...
  const openList = useCallback(async (listId: string) => {
    ...
    try {
      const result = await getLinkedInManagerLeadContacts(listId);
  ...
  const chosenHeader = useCallback((field: LeadField): string => (
    overrides[field] ?? preview?.mapping?.[field] ?? autoMapping[field] ?? ''
  ), [overrides, preview, autoMapping]);

  const mappingToSend = useCallback((next: FieldMapping): FieldMapping => {
  ...
  const runPreview = useCallback(async (target: File, mapping: FieldMapping, first: boolean) => {
    ...
    setBusy('preview');
    try {
      const result = await previewLinkedInManagerLeadCsv(target, Object.keys(mapping).length > 0 ? mapping : undefined);
  ...
  const takeFile = useCallback(async (next: File | null) => {
    ...
    setOverrides({});
    setSample(null);
    try { setSample(await readCsvHead(next)); } catch { /* the preview below reports what is wrong with it */ }
  ...
  const changeMapping = (field: LeadField, header: string) => {
    ...
    if (REQUIRED_FIELDS.every((required) => mapping[required])) void runPreview(file, mapping, false);
  ...
  const scrubExamples = useMemo(() => {
    ...
    sample.rows.forEach((row, index) => {
      ...
      acceptedIndex += 1;
      if (!clean) return;
      const before = `${firstHeader ? row[firstHeader] ?? '' : ''} ${lastHeader ? row[lastHeader] ?? '' : ''}`.replace(/\s+/g, ' ').trim();
  ...
  const blocker = !file ? ''
    : missingRequired.length > 0 ? 'Point first name, last name and company at a column before importing.'
      : !preview ? ''
        : preview.acceptedCount === 0 ? 'No row in this file has all three of first name, last name and company, so there is nothing to import yet.'
          : destination === 'new' && !newName.trim() ? 'Name the new list before importing.'
            : '';
  ...
  const runImport = async () => {
    ...
    setError('');
    try {
      const list = destination === 'new'
        ? await createLinkedInManagerLeadList({ name: newName.trim(), sourceKind: 'csv' })
        : destinationList!;
      const counts = await importLinkedInManagerLeadCsv(list.id, file, mappingToSend(overrides)) as ImportReport;
      setReport({ listId: list.id, listName: list.name, counts });
      setToast(counts.inserted > 0
  ...
  const removeContact = async (contact: LinkedInLeadContact) => {
    ...
    try {
      ...
      setArmed('');
      setListError('');
      setToast(`${contact.firstName} ${contact.lastName} removed. Their place in every campaign they were in went with them, along with any message a campaign was waiting on you to write for them. What Trevra already sent stays in the ledger.`);
      await loadLists();
      await onChanged();
    } catch (err) {
  ...
  const filtered = term
    ? contacts.filter((contact) => `${contact.firstName} ${contact.lastName} ${contact.company} ${contact.email ?? ''}`.toLowerCase().includes(term))
    : contacts;
  const shown = filtered.slice(0, visible);
  const draftValid = draft !== null && draft.firstName.trim() !== '' && draft.lastName.trim() !== '' && draft.company.trim() !== '';

  return <div className="lead-build">
    <section
      className="page-panel"
      onDragOver={(event) => { if (busy === '') { event.preventDefault(); setDragging(true); } }}
    ...
    >
      <div className="section-heading"><div>
        <h3 aria-level={2}><Users size={18} className="li-heading-icon" /> Add leads from a CSV</h3>
        <p>Names, companies and contact details from a file you already have. Nothing is stored until you import, and what you confirm here is exactly what lands — campaigns enrol people from these lists.</p>
      </div></div>

      {error && <div className="error-banner">{error}</div>}

      <div className="li-form-grid">
        <label>Add these leads to
          <select value={destination} disabled={busy !== ''} onChange={(event) => setDestination(event.target.value)}>
            <option value="new">A new list…</option>
            {lists.map((list) => <option key={list.id} value={list.id}>{list.name} · {plural(list.leadCount, 'lead')}</option>)}
          </select>
        </label>
        {destination === 'new' && <label>Name the new list
          <input value={newName} disabled={busy !== ''} placeholder="Q3 founders" onChange={(event) => setNewName(event.target.value)} />
        </label>}
      </div>
      ...
      <div className={dragging ? 'lead-drop is-dragging' : 'lead-drop'}>
        ...
        <div className="lead-drop-copy">
          ...
          <p>{file
            ...
            : 'Or choose one below. Up to 2 MB, .csv only — a file dropped here is read, not stored.'}</p>
        </div>
        <label className="secondary-button">
          {busy === 'preview' ? <LoaderCircle className="spin" size={14} /> : <FileUp size={14} />}
          ...
          <input
            ...
            hidden
            disabled={busy !== ''}
            onChange={(event) => { void takeFile(event.target.files?.[0] ?? null); event.target.value = ''; }}
          />
        </label>
      </div>

      {file && headers.length > 0 && <div className="lead-section">
        <h4>Which column is which</h4>
        <p className="li-hint">Matched from your headings. Change any of them and the rows below are read again. First name, last name and company have to point at a column — a lead without them is not a lead a campaign can write to.</p>
        <div className="li-form-grid lead-map-grid">
          {FIELDS.map((entry) => {
            ...
            return <label key={entry.field} className={missing ? 'lead-map-row is-missing' : 'lead-map-row'}>
              <span className="lead-map-name">{entry.label}{entry.required && <em>Required</em>}</span>
              <select
                value={chosen}
                disabled={busy !== ''}
                onChange={(event) => changeMapping(entry.field, event.target.value)}
              >
                {!chosen && <option value="">{entry.required ? 'Choose a column…' : 'Not imported'}</option>}
                {!entry.required && !auto && chosen && <option value="">Not imported</option>}
                {headers.map((header) => <option key={header} value={header}>{header || '(unnamed column)'}</option>)}
              </select>
              <span className="lead-map-note">{
                chosen && auto && chosen !== auto ? `Changed from “${auto}”`
                  : chosen && chosen === auto ? 'Matched by its heading'
                    : chosen ? 'Your choice'
                      : 'No column matched'
              }</span>
            </label>;
      ...
      {file && missingRequired.length > 0 && <div className="li-warn-block">
        <CircleAlert size={15} />
        <div>
          <strong>{missingRequired.length === 1 ? 'One column is still unanswered' : `${missingRequired.length} columns are still unanswered`}</strong>
          <ul>{missingRequired.map((entry) => <li key={entry.field}>Point <b>{entry.label}</b> at the column that holds it.</li>)}</ul>
        </div>
      </div>}

      {scrubExamples.length > 0 && <div className="lead-section">
        <h4>What the names will look like</h4>
        <p className="li-hint">Titles and qualifications — Dr, Prof, Mrs, PhD, MBA, MSc, CEO, LION and the rest of the usual list — along with emoji and stray punctuation, are removed before a lead is stored, so a message that opens with a first name opens with a name.</p>
        <ul className="lead-scrub-list">
          {scrubExamples.map((example, index) => <li key={`${example.before}:${index}`}>
            <span className={example.changed ? 'lead-scrub-raw is-changed' : 'lead-scrub-raw'}>{example.before}</span>
            <ArrowRight size={13} />
      ...
      {preview && preview.rejectedCount > 0 && <div className="lead-section">
        <h4>{plural(preview.rejectedCount, 'row')} will not be imported</h4>
        <p className="li-hint">Fix them in the file and drop it again, or import the rest and leave them. Row numbers count the heading as row 1.</p>
        <div className="li-table-scroll"><table className="li-table lead-rejects">
          <thead><tr><th>Row</th><th>Why</th></tr></thead>
          <tbody>{preview.rejected.slice(0, 8).map((entry) => <tr key={entry.row}>
            <td className="li-num">{entry.row}</td>
            <td>{entry.reason}</td>
          </tr>)}</tbody>
        </table></div>
        {preview.rejectedCount > 8 && <p className="li-hint">{plural(preview.rejectedCount - 8, 'other row')} for the same kinds of reason.</p>}
      </div>}

      <div className="panel-footer">
        <span>{blocker || (preview
          ...
          : 'Reading a file changes nothing. Importing writes the leads and contacts nobody.')}</span>
        <button className="primary-button" type="button" disabled={!canImport} onClick={() => void runImport()}>
          {busy === 'import' ? <LoaderCircle className="spin" size={14} /> : <Upload size={14} />}
          {preview ? ` Import ${plural(preview.acceptedCount, 'lead')}` : ' Import'}
        </button>
      </div>

      {report && <div className="lead-report" role="status">
        <strong><Check size={15} /> “{report.listName}” updated</strong>
        <ul className="lead-counts">
          <li><strong>{report.counts.inserted.toLocaleString()}</strong> added</li>
          <li><strong>{Math.max(0, report.counts.duplicates - (report.counts.reused ?? 0)).toLocaleString()}</strong> already in this list</li>
          <li><strong>{(report.counts.reused ?? 0).toLocaleString()}</strong> already yours in another list</li>
          <li><strong>{report.counts.rejected.length.toLocaleString()}</strong> rejected</li>
        </ul>
        <p className="li-hint">Somebody who was already in another list keeps their single record rather than being copied — one person is one lead, so a campaign cannot reach them twice.</p>
        <div className="mgr-actions">
          <button className="secondary-button" type="button" onClick={() => void openList(report.listId)}>
      ...
      <div className="lead-kinds">
        ...
        <dl>
          <div><dt>{SOURCE_LABELS.csv}</dt><dd>Built here, from a file you export from anywhere.</dd></div>
          <div><dt>{SOURCE_LABELS.linkedin_search}</dt><dd>Walked by the local browser worker on Lead sources, then added to a list from there.</dd></div>
    ...
    <section className="page-panel">
      <div className="section-heading">
        <div>
          ...
          <p>Read a list, fix a row, drop somebody who left. Editing a lead here changes them everywhere, including in campaigns they are already in.</p>
        </div>
        <button className="ghost-button" type="button" disabled={contactsLoading || listsLoading} onClick={() => void (async () => { await loadLists(); await openList(openListId); })()}>
          {contactsLoading || listsLoading ? <LoaderCircle className="spin" size={14} /> : <RefreshCw size={14} />} Refresh
        </button>
      </div>
      ...
      {!listsLoading && lists.length === 0
        ...
        : <>
          <div className="li-filter-row">
            <label>List
              <select value={openListId} disabled={contactsLoading} onChange={(event) => void openList(event.target.value)}>
                {lists.map((list) => <option key={list.id} value={list.id}>{list.name} · {plural(list.leadCount, 'lead')}</option>)}
              </select>
            </label>
            <label>Find someone
              <input value={filter} placeholder="Name, company or email" onChange={(event) => { setFilter(event.target.value); setVisible(PAGE); }} />
            </label>
            {openMeta && <span className="li-chip">From {SOURCE_LABELS[openMeta.sourceKind]}</span>}
          </div>

          {contactsLoading
            ? <p className="empty-copy">Reading this list…</p>
            : contacts.length === 0
              ? <p className="empty-copy">Nothing in this list yet. Import a CSV above and it fills up.</p>
              : <>
                ...
                <p className="li-hint">
                  {plural(filtered.length, 'lead')}{term ? ` of ${plural(contacts.length, 'lead')} read` : ''} in “{openMeta?.name ?? 'this list'}”.
                  {contactTotal > contacts.length && ` The first ${contacts.length.toLocaleString()} of ${plural(contactTotal, 'lead')} are shown — one read of a list returns no more than that, and Find someone searches the ones it returned.`}
                </p>
                {draft && <div className="lead-editor" ref={editorRef}>
                  <h4>Editing {contacts.find((contact) => contact.id === draft.id)?.firstName} {contacts.find((contact) => contact.id === draft.id)?.lastName}</h4>
                  <div className="li-form-grid lead-edit-grid">
                    <label>First name<input value={draft.firstName} onChange={(event) => setDraft({ ...draft, firstName: event.target.value })} /></label>
                    <label>Last name<input value={draft.lastName} onChange={(event) => setDraft({ ...draft, lastName: event.target.value })} /></label>
                    ...
                    <label>Country<input value={draft.country} onChange={(event) => setDraft({ ...draft, country: event.target.value })} /></label>
                    <label className="li-span-2">LinkedIn profile URL<input value={draft.profileUrl} placeholder="https://www.linkedin.com/in/…" onChange={(event) => setDraft({ ...draft, profileUrl: event.target.value })} /></label>
                  </div>
                  <p className="li-hint">Names are cleaned on save the same way they are on import. First name, last name and company are required.</p>
                  <div className="mgr-actions">
                    <button className="primary-button" type="button" disabled={!draftValid || rowBusy !== ''} onClick={() => void saveContact()}>
                      {rowBusy === draft.id ? <LoaderCircle className="spin" size={14} /> : <Check size={14} />} Save this lead
                    </button>
                    <button className="secondary-button" type="button" disabled={rowBusy !== ''} onClick={() => setDraft(null)}><X size={14} /> Cancel</button>
                  </div>
                </div>}
                ...
                */}
                {armedContact && <div className="li-warn-block" ref={confirmRef}>
                  <CircleAlert size={15} />
                  <div>
                    <strong>Remove {armedContact.firstName} {armedContact.lastName} for good?</strong>
                    <p>
                      This deletes the lead, not just their line in “{openMeta?.name ?? 'this list'}”. Their place in
                      every campaign they are enrolled in goes with them, and so does any message a campaign was
                      waiting on you to write for them — both hang off this record and are removed with it. What
                      Trevra has already sent them stays in the ledger; nothing is unsent.
                    </p>
                    <div className="mgr-actions">
                      <button className="secondary-button" type="button" disabled={rowBusy !== ''} onClick={() => setArmed('')}>
                        <X size={14} /> Keep them
                      </button>
                      <button className="li-mini-button li-mini-danger" type="button" disabled={rowBusy !== ''} onClick={() => void removeContact(armedContact)}>
                        {rowBusy === armedContact.id ? <LoaderCircle className="spin" size={14} /> : <Trash2 size={14} />} Remove for good
                      </button>
                    </div>
                  </div>
                </div>}

                <div className="li-table-scroll"><table className="li-table lead-contacts">
                  <thead><tr>
                    <th>Name</th><th>Company</th><th>Email</th><th>Phone</th><th>Country</th><th>LinkedIn</th><th><span className="mgr-sr">Actions</span></th>
                  </tr></thead>
                  <tbody>{shown.map((contact) => {
                    ...
                    return (
                      <tr key={contact.id} className={editing ? 'lead-row-editing' : undefined}>
                        ...
                        <td>{contact.profileUrl
                          ? <a className="li-link" href={contact.profileUrl} target="_blank" rel="noreferrer">Profile</a>
                          : '—'}</td>
                        <td><div className="li-row-actions">
                          {armed === contact.id
                            ? <span className="li-hint">Confirm above</span>
                            : <>
                              <button className="li-mini-button" type="button" disabled={rowBusy !== '' || editing} onClick={() => startEdit(contact)}>
                                <Pencil size={12} /> Edit
                              </button>
                              <button
                                ...
                                onClick={() => {
                                  ...
                                  requestAnimationFrame(() => confirmRef.current?.scrollIntoView({ block: 'nearest' }));
                                }}
                              >
                ...
                {filtered.length > shown.length && <div className="mgr-actions">
                  <button className="secondary-button" type="button" onClick={() => setVisible(visible + PAGE)}>
                    Show {Math.min(PAGE, filtered.length - shown.length)} more
                  </button>
                </div>}
                {filtered.length === 0 && <p className="empty-copy">Nobody in this list matches “{filter.trim()}”.</p>}
              </>}
        </>}
    </section>
  </div>;
