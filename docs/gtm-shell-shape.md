...
| --- | --------------------------------------- | --------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | **Find** — who is worth talking to      | Source, Enrich, Score | Partial. The CSV-paste path (`POST /api/linkedin/targets/import`, driven from the Find people fold in `src/client/LinkedInLeads.tsx`) was removed along with the legacy Target-accounts screen. Real lead sourcing exists server-side (`src/server/linkedin/leads.ts:326` `listLeads`, `migrations/030_linkedin_lead_sources.sql`) with **no HTTP route and no screen**.                                 |
...
| `campaigns`                | `/outreach`                    | Folded into the Campaigns screen itself; no longer a separate route.                                                                                                                                                                                                     |
| `plan`                     | **deleted**                    | Retired with the legacy campaign screen.                                                                                                                                                                                                                                 |
| `queue`                    | `/outreach/queue`              | Unchanged.                                                                                                                                                                                                                                                               |
| —                          | `/outreach/replies`            | **New.** Blocked on backend — see §4, Wave C1. Do not ship as an empty tab.                                                                                                                                                                                              |
| `analytics`                | **deleted as a tab**           | Absorbed three ways: the funnel (`FunnelBars`, `src/client/LinkedInAnalyticsScreen.tsx`) moves to `/loop`; the per-campaign table moves under `/outreach`; the daily volume chart is already drawn on Seat (`src/client/LinkedInSafety.tsx`, "Volume and its variance"). |
...
(`docs/app-spec.md` §5). The primary action lives on `/outreach/new`.
...
| 4   | **Build one campaign**        | `GET /api/linkedin/manager/campaigns` non-empty                              | `/outreach`                                                                                                                                                                                                                                                                                                             |
| 5   | **Preview the plan**          | removed — `POST /api/linkedin/plan` and the Plan-preview screen were deleted | —                                                                                                                                                                                                                                                                                                                       |
...
9. **Does the stage bar show "Find" on hosted workspaces?**
   ...
   link it to the CSV import — but `POST /api/linkedin/targets/import` was removed
   along with the legacy Target-accounts screen. Lead sourcing is unconditionally off on hosted
