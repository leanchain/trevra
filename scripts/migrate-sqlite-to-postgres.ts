...
for (const [source, tables] of [
  [appSource, APP_TABLES],
  [authSource, AUTH_TABLES]
] as const) {
  try {
...
try {
  ...
  if (!dryRun && !allowConflicts && Number(existingWorkspaces?.count ?? 0) > 0) {
    throw new Error(
      'Target PostgreSQL already contains workspaces. Use a fresh database or pass --allow-conflicts for a non-destructive merge.'
    );
  }
  ...
  const report = dryRun
    ...
    : await db.transaction(async (tx) => {
        ...
        if (conflicts > 0 && !allowConflicts) {
          throw new Error(
            `Migration found ${conflicts} conflicting rows. The transaction was rolled back; retry with a fresh database or --allow-conflicts.`
          );
        }
...
async function exportSqlite(
  ...
  const { stdout } = await execFileAsync(
    'python3',
    [
      resolve('scripts/sqlite-export.py'),
      '--source',
      source,
      '--snapshot',
      snapshot,
...
async function backfillRequiredWorkspaceData(db: Db): Promise<void> {
  ...
  for (const workspace of workspaces) {
    await db
      .prepare(
        `
      INSERT INTO automation_rules (id,workspace_id,recommendation_type,mode,min_confidence,delay_minutes,enabled,created_at,updated_at)
      VALUES (?,?,?,?,?,?,?,?,?) ON CONFLICT(workspace_id,recommendation_type) DO NOTHING
    `
      )
      .run(id('rule'), workspace.id, 'stale_proposal', 'prepare', 0.85, 0, 1, now, now);
  }

  const demo = await db
    .prepare('SELECT id FROM workspaces WHERE id=?')
    .get<{ id: string }>('ws_demo');
  if (demo) {
    await db
      .prepare(
        `
...
function printReport(report: ReportRow[], backups: string, isDryRun: boolean): void {
  console.table(
    report
      ...
      .map((row) => ({
        ...
        table: row.table,
        rows: row.rows,
        inserted: isDryRun ? 'dry-run' : row.inserted,
  ...
  console.log(
    isDryRun
      ...
      : 'SQLite to PostgreSQL migration committed successfully.'
  );
}
